...
#
OCTAVE_VER=11.0.0
OCTAVE_SOURCE_DIR=octave
if ! test -d $OCTAVE_SOURCE_DIR ; then
    hg clone https://hg.octave.org/octave
    pushd $OCTAVE_SOURCE_DIR
    hg pull
    hg update
#    cat > octave-$OCTAVE_VER.patch.uue << 'EOF'
#begin-base64 644 octave-11.0.0.patch
#EOF
    # Patch
...
# Build octave
#
BUILD_OCTAVE_DEBUG=1
BUILD_OCTAVE_ADDRESS_SANITIZER=0

if test $BUILD_OCTAVE_DEBUG -eq 1 ; then
    OCTAVE_OPTFLAGS="-m64 -march=nehalem -Og -ggdb3"
else
    OCTAVE_OPTFLAGS=$OPTFLAGS
fi
...
if test $BUILD_OCTAVE_ADDRESS_SANITIZER -eq 1 ; then
    CONFIG_ADDRESS_SANITIZER_FLAGS="--enable-address-sanitizer-flags"
else
    CONFIG_ADDRESS_SANITIZER_FLAGS=""
fi
...
export CFLAGS="$OCTAVE_OPTFLAGS -I$OCTAVE_INCLUDE_DIR"
export CXXFLAGS="$OCTAVE_OPTFLAGS -I$OCTAVE_INCLUDE_DIR"
export FFLAGS="$OCTAVE_OPTFLAGS"
export LDFLAGS="-L$OCTAVE_LIB_DIR"
PKG_CONFIG_PATH=$OCTAVE_LIB_DIR/pkgconfig \
    ../$OCTAVE_SOURCE_DIR/configure -C \
    $CONFIG_ADDRESS_SANITIZER_FLAGS \
