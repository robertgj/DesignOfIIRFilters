...
#
cat > test.k2.ok << 'EOF'
k2 = [   0.0000000000,  -0.9135081371,   0.0000000000,   0.5546428077, ... 
         0.0000000000,  -0.0966755724,   0.0000000000,   0.0007462344, ... 
         0.0000000000,  -0.0020582170,   0.0000000000,   0.0015840384 ];
EOF
...
cat > test.p2.ok << 'EOF'
p2 = [   2.2748006759,   2.2748006759,   0.4836328786,   0.4836328786, ... 
         0.9036013965,   0.9036013965,   0.9956211212,   0.9956211212, ... 
         0.9963643654,   0.9963643654,   0.9984172142,   0.9984172142 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.p2.ok"; fail; fi

cat > test.c2.ok << 'EOF'
c2 = [   0.0435189929,   0.0508921401,   0.2428519496,   0.2882277487, ... 
         0.1877845013,   0.2854412265,   0.6862739758,  -0.5909939643, ... 
