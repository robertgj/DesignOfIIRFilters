...
#
cat > test.k2.ok << 'EOF'
k2 = [   0.0000000000,  -0.9135081367,   0.0000000000,   0.5546428085, ... 
         0.0000000000,  -0.0966755734,   0.0000000000,   0.0007462344, ... 
         0.0000000000,  -0.0020582170,   0.0000000000,   0.0015840385 ];
EOF
...
cat > test.p2.ok << 'EOF'
p2 = [   2.2748006653,   2.2748006653,   0.4836328775,   0.4836328775, ... 
         0.9036013955,   0.9036013955,   0.9956211211,   0.9956211211, ... 
         0.9963643652,   0.9963643652,   0.9984172141,   0.9984172141 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.p2.ok"; fail; fi

cat > test.c2.ok << 'EOF'
c2 = [   0.0435189930,   0.0508921403,   0.2428519500,   0.2882277492, ... 
         0.1877845016,   0.2854412268,   0.6862739759,  -0.5909939644, ... 
