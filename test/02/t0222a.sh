...
#
cat > test.k2.ok << 'EOF'
k2 = [   0.0000000000,  -0.8102341455,   0.0000000000,   0.2650652146, ... 
         0.0000000000,  -0.0337238685,   0.0000000000,   0.0012334068, ... 
         0.0000000000,  -0.0003045749,   0.0000000000,   0.0001127069 ];
EOF
...
cat > test.p2.ok << 'EOF'
p2 = [   2.2722585931,   2.2722585931,   0.7356977255,   0.7356977255, ... 
         0.9652314603,   0.9652314603,   0.9983506724,   0.9983506724, ... 
         0.9995828052,   0.9995828052,   0.9998872994,   0.9998872994 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.p2.ok"; fail; fi

cat > test.c2.ok << 'EOF'
c2 = [  -0.0437999656,  -0.0526513028,  -0.1505372374,  -0.1845701677, ... 
        -0.1806515880,  -0.2725741449,  -0.6912580882,   0.5823105072, ... 
         0.1563223662,   0.0717396140,   0.0366465419,   0.0185433520, ... 
         0.0085430058 ];
EOF
