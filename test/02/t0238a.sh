...
#
cat > test.k2.ok << 'EOF'
k2 = [   0.0000000000,  -0.9167035716,   0.0000000000,   0.4392873057, ... 
         0.0000000000,  -0.0007273230,   0.0000000000,   0.0007326388, ... 
         0.0000000000,  -0.0002678789,   0.0000000000,  -0.0002076255 ];
EOF
...
cat > test.p2.ok << 'EOF'
p2 = [   2.9882723086,   2.9882723086,   0.6229538645,   0.6229538645, ... 
         0.9980664054,   0.9980664054,   0.9987925862,   0.9987925862, ... 
         0.9995246086,   0.9995246086,   0.9997923960,   0.9997923960 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.p2.ok"; fail; fi

cat > test.c2.ok << 'EOF'
c2 = [  -0.0299128593,  -0.0359368816,  -0.1941483782,  -0.2289356491, ... 
        -0.1776971239,  -0.2636817414,  -0.6863245322,   0.5868553485, ... 
         0.1562568342,   0.0693239234,   0.0424953763,   0.0186963704, ... 
         0.0111132498 ];
EOF
