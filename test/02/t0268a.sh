...
depends="test/schurOneMAPlattice_frm_hilbert_socp_slb_test.m test_common.m delayz.m \
...
schurOneMAPlattice2Abcd.m tf2pa.m H2Asq.m H2P.m H2T.m \
...
k2 = [  -0.5737703476,  -0.1357938232,  -0.0532669258,  -0.0211104590, ... 
        -0.0087692433 ]';
EOF
...
u2 = [  -0.0009134792,  -0.0025391780,  -0.0070956291,  -0.0128180955, ... 
        -0.0309769238,  -0.0343112020,  -0.0517560089,  -0.0570162087, ... 
         0.4398702228 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_u2_coef.m"; fail; fi

cat > test_v2_coef.m << 'EOF'
v2 = [   0.0065417823,   0.0043831468,   0.0072059953,   0.0020799886, ... 
        -0.0078694031,  -0.0311618598,  -0.0808644219,  -0.3144095684 ]';
EOF
