...
#
cat > test.s10.ok << 'EOF'
s10_1 = [   1.1196398731,   0.3624343251,  -0.3220924766,  -0.1769573752, ... 
            0.0486753449,   0.0336079236,   0.0039222825,  -0.0116730975, ... 
           -0.0063407539,   0.0023808057 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s10.ok"; fail; fi

cat > test.s11.ok << 'EOF'
s11_1 = [   0.9951216643,   1.1445427054,   1.1520886042,   1.0667924436, ... 
            0.9801307006,   1.0779019808,   0.9676719953,   1.0151031789, ... 
            1.0483584058,   0.7026579324 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s11.ok"; fail; fi

cat > test.s20.ok << 'EOF'
s20_1 = [  -0.9162622619,   0.6172899811,  -0.3831398413,   0.2518421296, ... 
           -0.1810060269,   0.0441000000,   0.0000000000,   0.0000000000, ... 
            0.0000000000,   0.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s20.ok"; fail; fi

cat > test.s00.ok << 'EOF'
s00_1 = [   0.9148243758,   0.9990000000,   0.8956740690,   0.7327653171, ... 
            0.9469556330,   0.3549335601,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s00.ok"; fail; fi

cat > test.s02.ok << 'EOF'
s02_1 = [   0.9162622619,  -0.6172899811,   0.3831398413,  -0.2518421296, ... 
            0.1810060269,  -0.0441000000,  -0.0000000000,  -0.0000000000, ... 
           -0.0000000000,  -0.0000000000 ];
EOF
...
cat > test.s22.ok << 'EOF'
s22_1 = [   0.9148243758,   0.9990000000,   0.8956740690,   0.7327653171, ... 
            0.9469556330,   0.3549335601,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
