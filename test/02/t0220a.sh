...
#
cat > test.s10.ok << 'EOF'
s10_1 = [   0.9641044962,   0.2517271628,  -0.1402747336,  -0.0967251317, ... 
            0.0323485553,   0.0338068797,   0.0035450518,  -0.0127709468, ... 
           -0.0060308320,   0.0018955955 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s10.ok"; fail; fi
cat > test.s11.ok << 'EOF'
s11_1 = [   0.8058361211,   1.0045805217,   1.0761984981,   1.0459755561, ... 
            1.0752037410,   0.9912742840,   1.0113710058,   1.0728310911, ... 
            0.9372142699,   0.6818730428 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s11.ok"; fail; fi
cat > test.s20.ok << 'EOF'
s20_1 = [  -0.8789392480,   0.5177732377,  -0.4172413376,   0.4454107324, ... 
           -0.1900019043,   0.0441000000,   0.0000000000,   0.0000000000, ... 
            0.0000000000,   0.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s20.ok"; fail; fi
cat > test.s00.ok << 'EOF'
s00_1 = [   0.9990000000,   0.8306681114,   0.6492241841,   0.9450224263, ... 
            0.9990000000,   0.6230049470,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s00.ok"; fail; fi
cat > test.s02.ok << 'EOF'
s02_1 = [   0.8789392480,  -0.5177732377,   0.4172413376,  -0.4454107324, ... 
            0.1900019043,  -0.0441000000,  -0.0000000000,  -0.0000000000, ... 
           -0.0000000000,  -0.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s02.ok"; fail; fi
cat > test.s22.ok << 'EOF'
s22_1 = [   0.9990000000,   0.8306681114,   0.6492241841,   0.9450224263, ... 
            0.9990000000,   0.6230049470,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
