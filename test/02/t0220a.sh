...
#
cat > test.s10.ok << 'EOF'
s10_1 = [   1.1196398729,   0.3624343251,  -0.3220924765,  -0.1769573752, ... 
...
cat > test.s11.ok << 'EOF'
s11_1 = [   0.9951216645,   1.1445427055,   1.1520886043,   1.0667924436, ... 
            0.9801307006,   1.0779019809,   0.9676719954,   1.0151031790, ... 
            1.0483584059,   0.7026579324 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s11.ok"; fail; fi

cat > test.s20.ok << 'EOF'
s20_1 = [  -0.9162622617,   0.6172899811,  -0.3831398413,   0.2518421296, ... 
...
cat > test.s00.ok << 'EOF'
s00_1 = [   0.9148243757,   0.9990000000,   0.8956740690,   0.7327653171, ... 
            0.9469556330,   0.3549335600,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s00.ok"; fail; fi

cat > test.s02.ok << 'EOF'
s02_1 = [   0.9162622617,  -0.6172899811,   0.3831398413,  -0.2518421296, ... 
            0.1810060269,  -0.0441000000,  -0.0000000000,  -0.0000000000, ... 
           -0.0000000000,  -0.0000000000 ];
EOF
...
cat > test.s22.ok << 'EOF'
s22_1 = [   0.9148243757,   0.9990000000,   0.8956740690,   0.7327653171, ... 
            0.9469556330,   0.3549335600,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
