...
#
# the octave-6.2.0 BLAS output should look like this
#
...
gfortran -Wall -o dgesvd_test_blas dgesvd_test.f \
         -L/usr/local/octave-6.2.0/lib -lblas -llapack
if [ $? -ne 0 ]; then \
    echo "Failed to compile dgesvd_test.f with libblas"; fail; fi

echo "LD_LIBRARY_PATH=/usr/local/octave-6.1.0/lib ldd dgesvd_test_blas" \
     > test.blas.ldd
LD_LIBRARY_PATH=/usr/local/octave-6.2.0/lib ldd dgesvd_test_blas >> test.blas.ldd

for k in `seq 1 100`;do \
    LD_LIBRARY_PATH=/usr/local/octave-6.2.0/lib \
