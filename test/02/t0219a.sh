...
s11_2 = [   0.9878553405,   0.8875529710,   1.2093774019,   0.7045312475, ... 
            1.3547246779,   1.1228481020,   1.4414340640,   1.1283195275, ... 
            0.8015521006,   0.6008716704 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s10.ok"; fail; fi

cat > test.s20.ok << 'EOF'
s20_2 = [  -0.9815911629,   0.6189392534,   0.1898969101,   0.7932273591, ... 
            0.5361454725,   0.0441000000,   0.0000000000,   0.0000000000, ... 
            0.0000000000,   0.0000000000 ];
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.s20.ok"; fail; fi

cat > test.s00.ok << 'EOF'
s00_2 = [   0.5349949481,   0.5689886263,   0.6468637671,   0.9552626051, ... 
...
s22_2 = [   0.8350750503,   0.9402625167,   0.9364552848,   0.9176192741, ... 
           -0.5544928935,   0.9990271218,   1.0000000000,   1.0000000000, ... 
            1.0000000000,   1.0000000000 ];
EOF
