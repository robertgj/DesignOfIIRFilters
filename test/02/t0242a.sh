...
#
cat > test.N0.ok << 'EOF'
N0 = [   0.0255776792,   0.0285640822,   0.0341142656,   0.0396655438, ... 
         0.0703996362,   0.2377762296,   0.0423863639,  -0.0133872668, ... 
        -0.0334788968,  -0.0398232699,  -0.0408510945,  -0.0050094159 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.N0.ok"; fail; fi

cat > test.D0.ok << 'EOF'
D0 = [   1.0000000000,  -0.0611946589,  -0.1229369838,  -0.1530780275, ... 
        -0.1351267200,  -0.1621389057,   0.0195591605,   0.0080472869, ... 
         0.0029853491,  -0.0023458469,  -0.0011912791,  -0.0047377809 ]';
EOF
