...
k2 = [   0.9969831138,   0.9969895356,   0.9970315993,   0.9970118952, ... 
         0.9969788825,   0.9969480090,   0.9968839642,   0.9969770913, ... 
         0.9882152183,   0.9308844656,   0.9759336637,   0.9802783957, ... 
         0.8896151518,   0.9639768865,   0.9969829906,   0.9839228780, ... 
         0.0903565883 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.ok.k2"; fail; fi
cat > test.ok.khat2 << 'EOF'
khat2 = [  -0.0021120378,   0.0094498350,   0.0344582318,   0.0297338829, ... 
           -0.0012620299,   0.0207739548,   0.0908277630,   0.0566317673, ... 
           -0.1728219179,  -0.3743031024,  -0.2321810068,   0.2134347939, ... 
            0.4637645671,   0.2776739351,  -0.0785613118,  -0.1960779160, ... 
            0.9968663441 ]';
EOF
