...
Exact & 0.001670 & & \\
12-bit 3-signed-digit(Lim)&0.001735 & 131 & 79 \\
12-bit 3-signed-digit(branch-and-bound)&0.000971 & 122 & 71 \\
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_12_nbits_cost.ok"; fail; fi

cat > test_12_nbits_k_min.ok << 'EOF'
k_min = [      -34,     1192,       34,     -296, ... 
               -20,      117,       13,      -54, ... 
               -14,       20 ]'/2048;
EOF
