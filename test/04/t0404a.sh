...
Exact & 0.001653 & & \\
12-bit 3-signed-digit(Lim)&0.001792 & 122 & 71 \\
12-bit 3-signed-digit(branch-and-bound)&0.000994 & 121 & 71 \\
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_12_nbits_cost.ok"; fail; fi

cat > test_12_nbits_k_min.ok << 'EOF'
k_min = [      -20,     1188,       36,     -295, ... 
                -8,      119,       16,      -52, ... 
                -7,       21 ]'/2048;
EOF
