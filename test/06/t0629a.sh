...
Da0 = [   1.0000000000,   0.5058479323,  -0.4824576958,   0.1937883781, ... 
          0.0404141638,  -0.1480559878,   0.1140369200,  -0.0328117358 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_Da0_coef.m"; fail; fi

cat > test_Db0_coef.m << 'EOF'
Db0 = [   1.0000000000,  -0.1327333277,   0.0685639801,   0.7853104198, ... 
         -0.6990618754,   0.0638987627,   0.3505858005,  -0.2957447120, ... 
          0.0893779836 ]';
EOF
