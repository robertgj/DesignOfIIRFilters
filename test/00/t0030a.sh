...
#
cat > test.a.ok << 'EOF'
a = [  -0.0021092647,   0.0004069329,   0.0076883395,   0.0051931733, ... 
       -0.0115231077,  -0.0210543663,   0.0019486082,   0.0417050319, ... 
        0.0578424346,   0.0390044901,   0.0123776721 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed x.a output cat"; fail; fi
cat > test.d.ok << 'EOF'
d = [   1.0000000000,  -2.4277939777,   3.0446749930,  -2.3351332888, ... 
        1.1383562669,  -0.3371599412,   0.0468622433 ]';
EOF
