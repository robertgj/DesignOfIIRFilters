...
#
cat > test.a.ok << 'EOF'
a = [  -0.0021092647,   0.0004069327,   0.0076883393,   0.0051931734, ... 
       -0.0115231074,  -0.0210543663,   0.0019486078,   0.0417050316, ... 
        0.0578424349,   0.0390044906,   0.0123776724 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed x.a output cat"; fail; fi
cat > test.d.ok << 'EOF'
d = [   1.0000000000,  -2.4277939753,   3.0446749910,  -2.3351332890, ... 
        1.1383562682,  -0.3371599421,   0.0468622435 ]';
EOF
