...
#
cat > test.N0.ok << 'EOF'
N0 = [  -0.0032171900,   0.0048721555,  -0.0077369793,   0.0154723646, ... 
        -0.0438655401,   0.4031186434,  -0.3994879901,  -0.0057619719, ... 
         0.0347748536 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.N0.ok"; fail; fi

cat > test.D0.ok << 'EOF'
D0 = [   1.0000000000,  -0.1247251301,  -0.0063558654,  -0.0041285321, ... 
        -0.0027140967 ]';
EOF
