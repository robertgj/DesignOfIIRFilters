...
#
cat > test.N0.ok << 'EOF'
N0 = [  -0.0032171726,   0.0048721546,  -0.0077369551,   0.0154723483, ... 
        -0.0438655501,   0.4031186657,  -0.3994880085,  -0.0057617957, ... 
         0.0347746932 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test.N0.ok"; fail; fi

cat > test.D0.ok << 'EOF'
D0 = [   1.0000000000,  -0.1247246918,  -0.0063558871,  -0.0041285265, ... 
        -0.0027141109 ]';
EOF
