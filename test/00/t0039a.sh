...
Da0 = [   1.0000000000,   0.0381258232,  -0.7377949275,   0.6798101137, ... 
          0.2903923277,  -0.3307496468 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_Da0_coef.m"; fail; fi

cat > test_Db0_coef.m << 'EOF'
Db0 = [   1.0000000000,  -0.5407488794,  -0.4263354267,   1.3158892889, ... 
         -0.2731547899,  -0.4215207665,   0.3473522239 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_Db0_coef.m"; fail; fi

cat > test_flat_Da0_coef.m << 'EOF'
Da0 = [   1.0000000000,   0.6973144853,  -0.2973771278,  -0.3128420155, ... 
         -0.1821558466,   0.0543123611,   0.0871588736,  -0.1041522117, ... 
          0.1846963896,   0.0438727063,  -0.1319673426,   0.0451571855 ]';
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_flat_Da0_coef.m"; fail; fi

cat > test_flat_Db0_coef.m << 'EOF'
Db0 = [   1.0000000000,   0.1561607072,  -0.3134975219,   0.3175788967, ... 
          0.1301916215,   0.0786625975,  -0.0643412048,  -0.1837391014, ... 
          0.2691891911,  -0.0895946065,  -0.1359558855,   0.1337931720, ... 
         -0.0581830264 ]';
EOF
