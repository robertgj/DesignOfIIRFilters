...
Exact & 0.001704 & & \\
16-bit 3-signed-digit(Ito)& 0.001692 & 139 & 87 \\
16-bit 3-signed-digit(SOCP-relax) & 0.001186 & 136 & 84 \\
EOF
if [ $? -ne 0 ]; then echo "Failed output cat test_16_nbits_cost.ok"; fail; fi

cat > test_16_nbits_k_allocsd.ok << 'EOF'
k_allocsd_digits = [  2,  3,  1,  4, ... 
                      2,  6,  6,  3, ... 
                      1,  6 ]';
EOF
