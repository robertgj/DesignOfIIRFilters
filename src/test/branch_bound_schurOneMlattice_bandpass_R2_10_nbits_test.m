% branch_bound_schurOneMlattice_bandpass_R2_10_nbits_test.m
% Copyright (C) 2025 Robert G. Jenssen

% Branch-and-bound search of a pipelined Schur one-multiplier lattice bandpass
% filter response with 10-bit 3-signed-digit coefficients and R=2

test_common;

strf="branch_bound_schurOneMlattice_bandpass_R2_10_nbits_test";
...
% Bandpass R=2 filter specification for schurOneMlattice_sqp_slb_bandpass_R2_test.m
...
schurOneMlattice_sqp_slb_bandpass_R2_test_N2_coef;
schurOneMlattice_sqp_slb_bandpass_R2_test_D2_coef;
[k0,epsilon0,p0,c0]=tf2schurOneMlattice(N2,D2);
clear N2 D2;
...
do
  ...
  Esq=schurOneMlatticeEsq(kc_b(Rk),epsilon0,p0,kc_b(Rc)/cscale, ...
                          wa,Asqd,Wa,wt,Td,Wt);
  printf("Found Esq=%g\n",Esq); 
...
if max(abs((abs(H_min).^2)-Asq)) > 20*eps
  error("max(abs((abs(H_min).^2)-Asq))(%g*eps) > 20*eps", ...
        max(abs((abs(H_min).^2)-Asq))/eps);
endif
T_min=delayz(N_min,D_min,wt);
T=schurOneMlatticeT(wt,k_min,epsilon0,p_min,c_min);
if max(abs(T_min-T)) > 1000*eps
  error("max(abs(T_min-T))(%g*eps) > 1000*eps", ...
        max(abs(T_min-T))/eps);
endif
...
fprintf(fid,"cscale=%g %% Scaling for c tap coefficients\n",cscale);
