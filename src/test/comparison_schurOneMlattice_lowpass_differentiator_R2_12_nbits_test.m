...
pop_Csq=schurOneMlatticeAsq(wa,pop_k,pop_epsilon,p_ones,pop_c);
pop_A=sqrt(pop_Csq).*Azm1;
pop_P=schurOneMlatticeP(wp,pop_k,pop_epsilon,p_ones,pop_c)+Pzm1;
pop_T=schurOneMlatticeT(wt,pop_k,pop_epsilon,p_ones,pop_c)+Tzm1;
pop_dCsqdw=schurOneMlatticedAsqdw(wd,pop_k,pop_epsilon,p_ones,pop_c);
pop_dAsqdw=(pop_Csq(Rdp).*dAzm1sqdw(Rdp))+(pop_dCsqdw.*Azm1sq(Rdp));
...
pop_Esq=schurOneMlatticeEsq ...
          (pop_k,pop_epsilon,p_ones,pop_c, ...
           wa,Asqd./Azm1sq,Wa,wt,Td-Tzm1,Wt,wp,Pd-Pzm1,Wp,wd,Cd,Wd);

% Plot amplitude pass-band response error for signed-digits
...
ha=plot(wa*0.5/pi,[exact_A,bandb_A,socp_A,pop_A]-Ad);
hls={"-",":","--","-."};
for l=1:4
  set(ha(l),"linestyle",hls{l});
endfor
...
% Plot amplitude stop-band response error for search methods
ha=plot(wa*0.5/pi,[exact_A,bandb_A,socp_A,pop_A]-Ad);
hls={"-",":","--","-."};
for l=1:4
  set(ha(l),"linestyle",hls{l});
...
% Plot phase pass-band response for signed-digits
ha=plot(wp*0.5/pi,([exact_P,sd_P,sd_Lim_P]+(wp*tp))/pi);
hls={"-",":","--","-."};
for l=1:3
  set(ha(l),"linestyle",hls{l});
endfor
...
ha=plot(wp*0.5/pi,([exact_P,bandb_P,socp_P,pop_P]+(wp*tp))/pi);
hls={"-",":","--","-."};
for l=1:4
  set(ha(l),"linestyle",hls{l});
endfor
...
ha=plot(wt*0.5/pi,[exact_T,sd_T,sd_Lim_T]);
hls={"-",":","--","-."};
for l=1:3
  set(ha(l),"linestyle",hls{l});
endfor
...
% Plot dAsqdw pass-band response for signed-digits
ha=plot(wd*0.5/pi,[exact_dAsqdw,sd_dAsqdw,sd_Lim_dAsqdw]-dAsqddw(Rdp));
hls={"-",":","--","-."};
for l=1:3
  set(ha(l),"linestyle",hls{l});
endfor
...
% Plot dAsqdw pass-band response for search methods
ha=plot(wd*0.5/pi, ...
        [exact_dAsqdw,bandb_dAsqdw,socp_dAsqdw,pop_dAsqdw]-dAsqddw(Rdp));
hls={"-",":","--","-."};
for l=1:4
  set(ha(l),"linestyle",hls{l});
endfor
...
fprintf(fid, ...
        "Exact &%8.2e&%8.2e&%8.2e&%8.2e&%8.2e&&\\\\\n", ...
...
fprintf(fid, ...
        "POP-relaxation &%8.2e&%8.2e&%8.2e&%8.2e&%8.2e&%d&%d\\\\\n", ...
        pop_Esq,pop_A_pass_error,pop_A_stop_error, ...
