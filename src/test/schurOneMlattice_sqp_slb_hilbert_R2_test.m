% schurOneMlattice_sqp_slb_hilbert_R2_test.m
% R=2 Schur one-multiplier lattice implementation of a Hilbert filter
% with denominator polynomial having coefficients only for z^2 terms
% Copyright (C) 2017-2025 Robert G. Jenssen

test_common;

strf="schurOneMlattice_sqp_slb_hilbert_R2_test";
...
% End of hack
axis(ax(1),[-0.5 0.5  0.5+(0.1*[-1,1])]);
axis(ax(2),[-0.5 0.5 -0.5+(0.1*[-1,1])]);
...
fprintf(fid,"Wap=%g %% Amplitude response weight\n",Wap);
fprintf(fid,"Wat=%g %% Amplitude transition weight\n",Wat);
