% iir_piqp_slb_hilbert_R2_test.m
...
strf="iir_piqp_slb_hilbert_R2_test";
...
subplot(311);
plot(w*0.5/pi,[A0,Adl,Adu]);
strt=sprintf("Hilbert filter initial response : R=2,tp=%g,fpt=%g",tp,fpt);
title(strt);
ylabel("Amplitude");
...
%
printf("\nMMSE pass 1:\n");
[x1,Ex1,piqp_iter,func_iter,feasible] = ...
  iir_piqp_mmse([], ...
...
grid("on");
subplot(312);
Px1_plot=[Px1 Pdl Pdu]+(w*tp)+(pp*pi);
...
%
printf("\nPCLS pass 1:\n");
[d1,E,slb_iter,piqp_iter,func_iter,feasible] = ...
  iir_slb(@iir_piqp_mmse, ...
...
fprintf(fid,"tp=%d %% Nominal filter group delay(samples)\n",tp);
