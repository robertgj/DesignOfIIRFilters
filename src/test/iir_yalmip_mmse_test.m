...
fasl=0.05;fasu=0.25;dBas=30;Wasl=1;Wat=0;Wasu=2;
...
for solver={"sedumi","scs-direct","scs-indirect","sdpt3"}
  
  solver_str=solver{1};
  printf("\n\nTesting %s solver\n\n",solver_str)
  solver_strf=strcat(strf,"_",solver_str);

  if strcmp(solver_str,"sedumi")
    solver_options=sdpsettings("solver",solver_str);
  elseif strcmp(solver_str, "sdpt3")
    solver_options=sdpsettings("solver",solver_str);
  elseif strcmp(solver_str,"scs-direct") || strcmp(solver_str,"scs-indirect")
    solver_options=sdpsettings("solver",solver_str,"scs.max_iters",1e4, ...
                               "scs.eps_abs",1e-9,"scs.eps_rel",1e-9);
  else
    error("Unknown solver %s!",solver_str);
  endif
  
  % MMSE pass
  feasible=0;
  try
    [x1,E,yalmip_iter,func_iter,feasible] = ...
  ...
  print_polynomial(x1,"x1",strcat(solver_strf,"_x1_coef.m"));
