% schurOneMAPlattice_frmAsq_test.m
...
%
[Asq,gradAsq]=schurOneMAPlattice_frmAsq([],k0,epsilon0,p0,u0,v0,Mmodel,Dmodel);
...
%
Hp=freqz(n0,rM,w);
Asq=schurOneMAPlattice_frmAsq(w,k0,epsilon0,p0,u0,v0,Mmodel,Dmodel);
Asqp=abs(Hp).^2;
if max(abs(Asq-Asqp)) > 1000*eps
  error("max(abs(Asq-Asqp))>1000*eps (%d*eps)",ceil(max(abs(Asq-Asqp))/eps));
endif
...
%
del=1e-6;
[Asq,gradAsq]=schurOneMAPlattice_frmAsq(w,k0,epsilon0,p0,u0,v0,Mmodel,Dmodel);
Nk=length(k0);
...
for l=1:Nk
  ...
  AsqP=schurOneMAPlattice_frmAsq(w,k0+delkon2,epsilon0,p0,u0,v0,Mmodel,Dmodel);
  AsqM=schurOneMAPlattice_frmAsq(w,k0-delkon2,epsilon0,p0,u0,v0,Mmodel,Dmodel);
...
if max(max(abs(diff_gradAsq(1:nap,:)))) > del/100;
  error("max(max(abs(diff_gradAsq(1:nap,:))))(%g*del) > del/100", ...
        max(max(abs(diff_gradAsq(1:nap,:))))/del);
endif
% Whole band
if verbose
...
if max(max(abs(diff_gradAsq))) > del/100;
  error("max(max(abs(diff_gradAsq)))(%g*del) > del/100",
        max(max(abs(diff_gradAsq)))/del);
endif
...
for l=1:Nu
  ...
  AsqP=schurOneMAPlattice_frmAsq(w,k0,epsilon0,p0,u0+deluon2,v0,Mmodel,Dmodel);
  AsqM=schurOneMAPlattice_frmAsq(w,k0,epsilon0,p0,u0-deluon2,v0,Mmodel,Dmodel);
  approx_gradAsq(:,l)=(AsqP-AsqM)/del;
...
if max(max(abs(diff_gradAsq(1:nap,:)))) > del/800;
  error("max(max(abs(diff_gradAsq(1:nap,:))))(%g*del) > del/800", ...
        max(max(abs(diff_gradAsq(1:nap,:))))/del);
endif
% Whole band
if verbose
...
if max(max(abs(diff_gradAsq))) > del/800;
  error("max(max(abs(diff_gradAsq)))(%g*del) > del/800",
        max(max(abs(diff_gradAsq)))/del);
endif
...
for l=1:Nv
  ...
  AsqP=schurOneMAPlattice_frmAsq(w,k0,epsilon0,p0,u0,v0+delvon2,Mmodel,Dmodel);
  AsqM=schurOneMAPlattice_frmAsq(w,k0,epsilon0,p0,u0,v0-delvon2,Mmodel,Dmodel);
  approx_gradAsq(:,l)=(AsqP-AsqM)/del;
...
%
diary off
movefile schurOneMAPlattice_frmAsq_test.diary.tmp ...
         schurOneMAPlattice_frmAsq_test.diary;
