...
R=1;N=30
fap=0.15,dBap=3,Wap=1,Wat=ctol
ftp=0.14,td=10,tdr=0.6,Wtp=0.25
...
% Amplitude constraints on pass band and transition band
fa=(0:(nas-2))'*0.5/n;
wa=2*pi*fa;
...
% Sanity checks
nchks=[1,2,n-nas-1,n-nas,n-nas+1]';
printf("fs(nchks)'=[ ");printf("%7.5g ",fs(nchks)');printf("];\n");
printf("Sd(nchks)=[ ");printf("%7.5g ",Sd(nchks)');printf("];\n");
printf("Sdu(nchks)=[ ");printf("%7.5g ",Sdu(nchks)');printf("];\n");
printf("Sdl(nchks)=[ ");printf("%7.5g ",Sdl(nchks)');printf("];\n");
printf("Ws(nchks)=[ ");printf("%7.5g ",Ws(nchks)');printf("];\n");
...
strf="iir_socp_slb_fir_lowpass_alternate_test";
...
close
showResponsePassBands(0,fap,-0.6,0.2,x0,U,V,M,Q,R,strt);
...
% Pass band error compared to pure delay
P=iirP(wt,d1,U,V,M,Q,R);
...
% Done
toc;
save iir_socp_slb_fir_lowpass_alternate_test.mat N U V M Q R tol ctol ...
     fap dBap Wap ftp td tdr Wtp fas dBas Was bi b0 x0 d1

diary off
movefile iir_socp_slb_fir_lowpass_alternate_test.diary.tmp ...
         iir_socp_slb_fir_lowpass_alternate_test.diary;
