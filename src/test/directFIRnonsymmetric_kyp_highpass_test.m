...
delete("directFIRnonsymmetric_kyp_highpass_test.diary");
delete("directFIRnonsymmetric_kyp_highpass_test.diary.tmp");
diary directFIRnonsymmetric_kyp_highpass_test.diary.tmp
...
N=30,d=10,fas=0.15,fap=0.2,Esq_s=1e-4,Esq_max=1.1
...
N=%d,d=%d,fas=%g,fap=%g,Esq\\_s=%g",N,d,fas,fap,Esq_s);
title(strt);
...
title(strt);
subplot(312)
plot(w(nap:end)*0.5/pi, ...
     unwrap(mod((w(nap:end)*d)+unwrap(arg(H(nap:end))),2*pi))/(pi))
axis([fap 0.5]);
grid("on");
...
printf("max(abs(H(1:nas)))=%8.6f\n",max(abs(H(1:nas))));
printf("min(abs(H(1:nas)))=%8.6f\n",min(abs(H(1:nas))));
printf("max(abs(H(nas:nap)))=%8.6f\n",max(abs(H(nas:nap))));
printf("min(abs(H(nas:nap)))=%11.6g\n",min(abs(H(nas:nap))));
printf("max(abs(H(nap:end)))=%11.6g\n",max(abs(H(nap:end))));
...
toc;
diary off
movefile directFIRnonsymmetric_kyp_highpass_test.diary.tmp ...
         directFIRnonsymmetric_kyp_highpass_test.diary;
