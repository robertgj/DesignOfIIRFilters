% branch_bound_directFIRnonsymmetric_bandpass_hilbert_12_nbits_test.m
% Copyright (C) 2026 Robert G. Jenssen

% Branch-and-bound search for the 12-bit 3-signed-digit coefficients of a
% direct-form non-symmetric Hilbert bandpass filter

test_common;

strf="branch_bound_directFIRnonsymmetric_bandpass_hilbert_12_nbits_test";
...
% Wpp=5;Wtp=5; produces a better filter but I want them to be the same as in
% socp_relaxation_directFIRnonsymmetric_bandpass_hilbert_12_nbits_test.m
fasl=0.05;fapl=0.1;fapu=0.2;fasu=0.25;
Wasl=1;Watl=0.01;Wap=1;Watu=0.01;Wasu=2;
pp=3.5;fppl=0.12;fppu=0.18;Wpp=2;
tp=16;ftpl=0.12;ftpu=0.18;Wtp=1;
...
% Allocate signed digits with the heuristic of Lim et al.
ndigits_Lim=directFIRnonsymmetric_allocsd_Lim(nbits,ndigits, ...
                                              h,wa,Asqd,Wa,wt,Td,Wt,wp,Pd,Wp);
print_polynomial(ndigits_Lim,"ndigits_Lim","%2d");
h_allocsd_digits=int16(ndigits_Lim);
...
[h_Lim_sd,h_Lim_sdu,h_Lim_sdl]=flt2SD(h(:),nbits,h_allocsd_digits(:));
% Find signed-digit error
Esq_Lim_sd=directFIRnonsymmetricEsq(h_Lim_sd,wa,Asqd,Wa,wt,Td,Wt,wp,Pd,Wp);
printf("Esq_Lim_sd=%g\n",Esq_Lim_sd);
% Find the number of signed-digits used by h_Lim_sd
[h_Lim_sd_digits,h_Lim_sd_adders]=SDadders(h_Lim_sd,nbits);
printf("h_Lim_sd uses %d signed-digits\n",h_Lim_sd_digits);
printf("h_Lim_sd uses %d %d-bit adders for coefficient multiplications\n", ...
       h_Lim_sd_adders,nbits);

% Check for consistent upper and lower bounds
if any(h_Lim_sdl>h_Lim_sdu)
  error("found h_Lim_sdl>h_Lim_sdu");
endif
if any(h_Lim_sdl>h_Lim_sdu)
  error("found h_Lim_sdl>h_Lim_sdu");
endif
if any(h_Lim_sd(h_active)>h_Lim_sdu(h_active))
  error("found h_Lim_sd(h_active)>h_Lim_sdu(h_active)");
endif
if any(h_Lim_sdl(h_active)>h_Lim_sd(h_active))
  error("found h_Lim_sdl(h_active)>h_Lim_sd(h_active)");
endif
...
fprintf(fid,"%d-bit %d-signed-digit(Lim) & %10.2e & %d & %d \\\\\n", ...
        nbits,ndigits,Esq_Lim_sd,h_Lim_sd_digits,h_Lim_sd_adders);
...
% Plot phase response
P_all=[P_h,P_h_sd,P_h_Lim_sd,P_h_min];
...
% Plot delay response
T_all=[T_h,T_h_sd,T_h_Lim_sd,T_h_min];
...
print_polynomial(h_Lim_sd,"h_Lim_sd",strcat(strf,"_h_Lim_sd_coef.m"),nscale);
print_polynomial(h_min,"h_min",nscale);
print_polynomial(h_min,"h_min",strcat(strf,"_h_min_coef.m"),nscale);

% Save results
eval(sprintf(["save %s.mat ", ...
              "h h_sd h_Lim_sd h_min tol nbits ndigits ", ...
