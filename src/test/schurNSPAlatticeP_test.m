...
for m=1:2
  ...
  delA1s(1)=del/2;
  diff_PA1=zeros(length(wt),A1Ns*4);
  for l=1:A1Ns
    % A1s20
  ...
  delA2s(1)=del/2;
  diff_PA2=zeros(length(wt),A2Ns*4);
  for l=1:A2Ns
    % A2s20
    ...
    PA2s20M=schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                              A2s20-delA2s,A2s00,A2s02,A2s22,difference);
    diff_PA2(:,l)=(PA2s20P-PA2s20M)/del;
    % A2s00
    ...
    PA2s00M=schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                              A2s20,A2s00-delA2s,A2s02,A2s22,difference);
    diff_PA2(:,A2Ns+l)=(PA2s00P-PA2s00M)/del;
    % A2s02
    ...
    PA2s02M=schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                              A2s20,A2s00,A2s02-delA2s,A2s22,difference);
    diff_PA2(:,(2*A2Ns)+l)=(PA2s02P-PA2s02M)/del;
    % A2s22
    ...
    PA2s22M=schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                              A2s20,A2s00,A2s02,A2s22-delA2s,difference);
    diff_PA2(:,(3*A2Ns)+l)=(PA2s22P-PA2s22M)/del;
    % Shift delA2s
    delA2s=circshift(delA2s,1);
  endfor
  max_gradPA2_error=...
    max(max(abs(diff_PA2-gradP(:,((A1Ns*4)+1):((A1Ns+A2Ns)*4)))));
  ...
  delA1s=zeros(size(A1s20));
  delA1s(1)=del/2;
  diff_gradPA1=zeros(length(wt),A1Ns*4);
  for l=1:A1Ns
    % A1s20
    [PA1s20P,gradPA1s20P]=...
    ...
    [PA1s20M,gradPA1s20M]=...
      schurNSPAlatticeP(wt,A1s20-delA1s,A1s00,A1s02,A1s22, ...
                        A2s20,A2s00,A2s02,A2s22,difference);
    lindex=l;
    diff_gradPA1(:,lindex)=(gradPA1s20P(:,lindex)-gradPA1s20M(:,lindex))/del;
    % A1s00
    [PA1s00P,gradPA1s00P]=...
    ...
    [PA1s00M,gradPA1s00M]=...
      schurNSPAlatticeP(wt,A1s20,A1s00-delA1s,A1s02,A1s22, ...
                        A2s20,A2s00,A2s02,A2s22,difference);
    lindex=A1Ns+l;
    diff_gradPA1(:,lindex)=(gradPA1s00P(:,lindex)-gradPA1s00M(:,lindex))/del;
    % A1s02
    [PA1s02P,gradPA1s02P]=...
    ...
    [PA1s02M,gradPA1s02M]=...
      schurNSPAlatticeP(wt,A1s20,A1s00,A1s02-delA1s,A1s22, ...
                        A2s20,A2s00,A2s02,A2s22,difference);
    lindex=(2*A1Ns)+l;
    diff_gradPA1(:,lindex)=(gradPA1s02P(:,lindex)-gradPA1s02M(:,lindex))/del;
    % A1s22
    [PA1s22P,gradPA1s22P]=...
    ...
    [PA1s22M,gradPA1s22M]=...
      schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22-delA1s, ...
                        A2s20,A2s00,A2s02,A2s22,difference);
    lindex=(3*A1Ns)+l;
    diff_gradPA1(:,lindex)=(gradPA1s22P(:,lindex)-gradPA1s22M(:,lindex))/del;
    % Shift delA1s
    delA1s=circshift(delA1s,1);
  endfor
  max_diagHessPA1_error=max(max(abs(diff_gradPA1-diagHessP(:,1:(A1Ns*4)))));
  ...
  delA2s=zeros(size(A2s20));
  delA2s(1)=del/2;
  diff_gradPA2=zeros(length(wt),A2Ns*4);
  for l=1:A2Ns
    % A2s20
    [PA2s20P,gradPA2s20P]=...
    ...
    diff_gradPA2(:,lindex)=(gradPA2s20P(:,rindex)-gradPA2s20M(:,rindex))/del;
    % A2s00
    [PA2s00P,gradPA2s00P]=...
      schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                        A2s20,A2s00+delA2s,A2s02,A2s22,difference);
    ...
    lindex=A2Ns+l;
    rindex=lindex+(A1Ns*4);
    diff_gradPA2(:,lindex)=(gradPA2s00P(:,rindex)-gradPA2s00M(:,rindex))/del;
    % A2s02
    [PA2s02P,gradPA2s02P]=...
      schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                        A2s20,A2s00,A2s02+delA2s,A2s22,difference);
    ...
    lindex=(2*A2Ns)+l;
    rindex=lindex+(A1Ns*4);
    diff_gradPA2(:,lindex)=(gradPA2s02P(:,rindex)-gradPA2s02M(:,rindex))/del;
    % A2s22
    [PA2s22P,gradPA2s22P]=...
      schurNSPAlatticeP(wt,A1s20,A1s00,A1s02,A1s22, ...
                        A2s20,A2s00,A2s02,A2s22+delA2s,difference);
    ...
    lindex=(3*A2Ns)+l;
    rindex=lindex+(A1Ns*4);
    diff_gradPA2(:,lindex)=(gradPA2s22P(:,rindex)-gradPA2s22M(:,rindex))/del;
    % Shift delA2s
    delA2s=circshift(delA2s,1);
  endfor
  max_diagHessPA2_error=...
    max(max(abs(diff_gradPA2-diagHessP(:,((A1Ns*4)+1):((A1Ns+A2Ns)*4)))));
