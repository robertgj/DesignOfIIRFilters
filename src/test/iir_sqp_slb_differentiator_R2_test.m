...
strf="iir_sqp_slb_differentiator_R2_test";
...
% From tarczynski_differentiator_R2_test.m
tarczynski_differentiator_R2_test_D0_coef; 
tarczynski_differentiator_R2_test_N0_coef;
...
plot(w*0.5/pi,A0);
axis([0 0.5 -0.1 1.1]);
strI=sprintf("Differentiator initial response:R=%d,tp=%g",R,tp);
title(strI);
ylabel("Amplitude");
grid("on");
subplot(312);
...
Td=tp*ones(size(w));
Tdu=[(Td(1:nt2)+(tpr/2)); (Td((nt2+1):end)+10000)];
Tdl=[(Td(1:nt2)-(tpr/2)); (Td((nt2+1):end)-10000)];
Pd=(3*pi/2)-(w*tp);
Pdu=[(Pd(1:nt2)+(ppr*pi/2)); (Pd((nt2+1):end)+10000)];
Pdl=[(Pd(1:nt2)-(ppr*pi/2)); (Pd((nt2+1):end)-10000)];
...
strP=sprintf(["Differentiator PCLS response:", ...
 "R=%d,ft1=%g,ft2=%g,Ar1=%g,Ar2=%g,tp=%g,tpr=%g,ppr=%g"],
...
fprintf(fid,"ppr=%g %% Nominal pass band phase(rad./$\\pi$))\n",pp);
...
eval(sprintf(["save %s.mat U V M Q R x0 x1 d1 N1 D1 ", ...
 "ftol ctol n ft1 ft2 Ar1 Ar2 Wap tp tpr Wtp pp ppr Wpp"],strf));
