...
strf="schurOneMPAlattice_socp_slb_lowpass_differentiator_alternate_test";
...
% Initial filter 
tarczynski_parallel_allpass_lowpass_differentiator_alternate_test_Da0_coef;
tarczynski_parallel_allpass_lowpass_differentiator_alternate_test_Db0_coef;
...
Csq1=schurOneMPAlatticeAsq(wa,A1k1,A1kones,A1kones,A2k1,A2kones,A2kones,df);
A1c=sqrt(Csq1);
A1=A1c.*Az;
P1c=schurOneMPAlatticeP(wp,A1k1,A1kones,A1kones,A2k1,A2kones,A2kones,df);
P1=P1c+Pz;
T1c=schurOneMPAlatticeT(wt,A1k1,A1kones,A1kones,A2k1,A2kones,A2kones,df);
T1=T1c+Tz;
dCsqdw1= ...
...
printf("\nPCLS pass :\n");
feasible=false;
[A1k2,A2k2,slb_iter,opt_iter,func_iter,feasible] = schurOneMPAlattice_slb ...
  (@schurOneMPAlattice_socp_mmse, ...
...
% Calculate the overall response
Csq2=schurOneMPAlatticeAsq(wa,A1k2,A1kones,A1kones,A2k2,A2kones,A2kones,df);
A2c=sqrt(Csq2);
A2=A2c.*Az;
P2c=schurOneMPAlatticeP(wp,A1k2,A1kones,A1kones,A2k2,A2kones,A2kones,df);
P2=P2c+Pz;
T2c=schurOneMPAlatticeT(wt,A1k2,A1kones,A1kones,A2k2,A2kones,A2kones,df);
T2=T2c+Tz;
dCsqdw2= ...
  schurOneMPAlatticedAsqdw(wd,A1k2,A1kones,A1kones,A2k2,A2kones,A2kones,df);
dAsqdw2=(Csq2(Rdp).*dAzsqdw(Rdp))+(dCsqdw2.*(Azsq(Rdp)));

% Find overall filter polynomials
[N2,D2]=schurOneMPAlattice2tf(A1k2,A1kones,A1kones,A2k2,A2kones,A2kones,df);

% Sanity check
Hc=freqz(conv(N2,Fz),D2,w);
if max(abs(abs(Hc)-A2)) > 1e3*eps
  error("max(abs(abs(Hc)-A2))(%g*eps) > 1e3*eps",max(abs(abs(Hc)-A2))/eps);
endif
Tc=delayz(conv(N2,Fz),D2,wt);
if max(abs(abs(Tc)-T2)) > 1e6*eps
  error("max(abs(abs(Tc)-T2))(%g*eps) > 1e6*eps",max(abs(abs(Tc)-T2))/eps);
endif
