% socp_relaxation_schurOneMlattice_bandpass_R2_10_nbits_test.m
% Copyright (C) 2025 Robert G. Jenssen

% Optimisation of Schur one-multiplier lattice bandpass filter response with
% 10-bit signed-digit coefficients and SOCP relaxation solution.

test_common;

strf="socp_relaxation_schurOneMlattice_bandpass_R2_10_nbits_test";
...
print_polynomial(k_allocsd_digits,"k_allocsd_digits","%2d");
...
print_polynomial(c_allocsd_digits,"c_allocsd_digits","%2d");
...
fprintf(fid,"%d-bit %d-signed-digit(SOCP-relax) & %6.4f & %d & %d \\\\\n", ...
...
ylabel("Amplitude(dB)");
axis([0 0.5 -50 -30]);
strt=sprintf(["Schur one-multiplier lattice bandpass R=2 filter stop-band ", ...
...
ylabel("Amplitude(dB)");
axis([0.1 0.2 -2 2]);
strt=sprintf(["Schur one-multiplier lattice bandpass R=2 filter pass-band ", ...
