% directFIRnonsymmetric_kyp_bmi_lowpass_test.m
% Copyright (C) 2023 Robert G. Jenssen

% SDP design of a direct-form FIR lowpass filter with the KYP lemma
% and YALMIP bmibnb. This script fails after YALMIP times out.
...
strf="directFIRnonsymmetric_kyp_bmi_lowpass_test";
...
c_p=2*cos(2*pi*fap);
Psi_z=[0,1;1,-c_p];
Pi22_z=zeros(N+1,N+1);
Pi22_z(N+1,N+1)=-Esq_z;
c_s=2*cos(2*pi*fas);
Psi_s=[0,-1;-1,c_s];
Pi22_s=zeros(N+1,N+1);
Pi22_s(N+1,N+1)=-Esq_s;
...
F_z=((AB')*(kron(Phi,P_z)+kron(Psi_z,Q_z))*AB) + Pi22_z + ((CD_d')*CD_d);

% Constraint on maximum stop band amplitude
P_s=sdpvar(N,N,"symmetric","real");
Q_s=sdpvar(N,N,"symmetric","real");
F_s=((AB')*(kron(Phi,P_s)+kron(Psi_s,Q_s))*AB) + Pi22_s + ((CD')*CD);
