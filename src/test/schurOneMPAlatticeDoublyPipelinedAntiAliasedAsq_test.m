...
strf="schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq_test";
...
for difference=[false,true]
  ...
  H=HA.*HB;
  if verbose
    printf("max(abs((abs(H).^2)-Asq))=%g*eps\n",max(abs((abs(H).^2)-Asq))/eps);
  endif
  if max(abs((abs(H).^2)-Asq)) > 200*eps
    error("max(abs((abs(H).^2)-Asq)) > 200*eps");
  endif

  %
  % Check the gradients of Asq wrt k
  %
  [~,gradAsq]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq...
                (wcheck,A1k,A2k,difference,B1k,B2k);
  del=1e-6;
  k=[A1k,A2k,B1k,B2k];
  delk=zeros(size(k));
  delk(1)=del/2;
  diff_Asqk=zeros(ncheck,Nk);
  for l=1:Nk
    AsqkP=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
            (wcheck,A1k+delk(RA1k),A2k+delk(RA2k),difference, ...
             B1k+delk(RB1k),B2k+delk(RB2k));
    AsqkM=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
            (wcheck,A1k-delk(RA1k),A2k-delk(RA2k),difference, ...
             B1k-delk(RB1k),B2k-delk(RB2k));
  ...
  if verbose
    printf("max(max(abs(diff_Asqk-gradAsq)))=%g*del\n", ...
           max(max(abs(diff_Asqk-gradAsq)))/del);
  endif
  if max(max(abs(diff_Asqk-gradAsq))) > del/200
    error("max(max(abs(diff_Asqk-gradAsq))) > del/200");
  endif

  %
  % Check the diagonal of the Hessian, diagHessAsq, wrt k
  %
  [~,~,diagHessAsq]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
                      (wcheck,A1k,A2k,difference,B1k,B2k);
  del=1e-6;
  k=[A1k,A2k,B1k,B2k];
  delk=zeros(size(k));
  delk(1)=del/2;
  diff_gradAsqk=zeros(ncheck,Nk);
  for l=1:Nk
    [~,gradAsqkP]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
                    (wcheck,A1k+delk(RA1k),A2k+delk(RA2k),difference, ...
                     B1k+delk(RB1k),B2k+delk(RB2k));
    [~,gradAsqkM]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
                    (wcheck,A1k-delk(RA1k),A2k-delk(RA2k),difference, ...
                     B1k-delk(RB1k),B2k-delk(RB2k));
    delk=circshift(delk,1);
  ...
  if verbose
    printf("max(max(abs(diff_gradAsqk-diagHessAsq)))=%g*del\n", ...
           max(max(abs(diff_gradAsqk-diagHessAsq)))/del);
  endif
  if max(max(abs(diff_gradAsqk-diagHessAsq))) > del/2
    error("max(max(abs(diff_gradAsqk-diagHessAsq))) > del/2");
  endif

  %
  % Check the Hessian of Asq wrt k
  %
  [~,~,~,hessAsq]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
                    (wcheck,A1k,A2k,difference,B1k,B2k);
  del=1e-6;
  k=[A1k,A2k,B1k,B2k];
  delk=zeros(size(k));
  delk(1)=del/2;
  diff_gradAsqk=zeros(ncheck,Nk,Nk);
  for l=1:Nk
    [~,gradAsqkP]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
                    (wcheck,A1k+delk(RA1k),A2k+delk(RA2k),difference, ...
                     B1k+delk(RB1k),B2k+delk(RB2k));
    [~,gradAsqkM]=schurOneMPAlatticeDoublyPipelinedAntiAliasedAsq ...
                    (wcheck,A1k-delk(RA1k),A2k-delk(RA2k),difference, ...
                     B1k-delk(RB1k),B2k-delk(RB2k));
    delk=circshift(delk,1);
    diff_gradAsqk(:,l,:)=(gradAsqkP-gradAsqkM)/del;
  endfor

  % Blocks on the diagonal
  max_diff_hessAsqk=max(max(max(abs(hessAsq-(diff_gradAsqk.*test_hess)))));
  if verbose
    printf("max_diff_hessAsqk=%g*del\n",max_diff_hessAsqk/del);
  endif
  if max_diff_hessAsqk > del/2
    error("max_diff_hessAsqk(%g*del) > del/2\n",max_diff_hessAsqk/del);
  endif

  % Blocks off the diagonal are expected to be zero
  max_diff_gradAsqk= ...
    max(max(max(abs((hessAsq-diff_gradAsqk).*(test_hess==0)))))*del;
  if verbose
    printf("max_diff_gradAsqk=%g\n",max_diff_gradAsqk);
  endif
  if max_diff_gradAsqk > 50*del
    error("max_diff_gradAsqk(%g) > 50*del\n",max_diff_gradAsqk);
  endif
