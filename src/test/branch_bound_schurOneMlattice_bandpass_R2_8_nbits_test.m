% branch_bound_schurOneMlattice_bandpass_R2_8_nbits_test.m
...
strf="branch_bound_schurOneMlattice_bandpass_R2_8_nbits_test";
...
fasl=0.05,fasu=0.25,Wasl=1e4,Wasu=1e7
...
strt=sprintf(["Schur OneM lattice bandpass R=2 filter ", ...
              "(nbits=%d) : fasl=%g,fasu=%g,Wasl=%g,Wasu=%g"], ...
             nbits,fasl,fasu,Wasl,Wasu);
...
ylabel("Amplitude(dB)");
axis([0.1 0.2 -2 1]);
strt=sprintf(["Schur OneM lattice bandpass R=2 filter (nbits=%d) : ", ...
              "fapl=%g,fapu=%g"], nbits,fapl,fapu);
title(strt);
legend("exact","s-d","s-d(BandB)");
...
strt=sprintf(["Schur OneM lattice bandpass R=2 filter (nbits=%d) ", ...
              ": ftpl=%g,ftpu=%g,tp=%g,Wtp=%g"],nbits,ftpl,ftpu,tp,Wtp);
title(strt);
legend("exact","s-d","s-d(BandB)");
legend("location","northeast");
legend("boxoff");
