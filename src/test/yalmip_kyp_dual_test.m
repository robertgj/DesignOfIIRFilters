...
fprintf(fid,"KYP lemma & %16.8g & %10.6f & %d & %d\\\\\n", ...
        KYP_Esq,sol.solvertime,nnz(sol.solverinput.A),numel(sol.solverinput.A)); 
...
fprintf(fid,"Dual of KYP lemma & %16.8g & %10.6f & %d & %d \\\\\n", ...
        Dual_Esq,sol.solvertime,nnz(sol.solverinput.A), ...
        numel(sol.solverinput.A));
...
ZR12=ZR(1:N,N+1);
ZR22=ZR(N+1,N+1);
Constraints=[ZR>=0,trace(Theta1*ZR)==-1];
...
fprintf(fid,"Reduced dual of KYP lemma & %16.8g & %10.6f & %d & %d\\\\\n", ...
...
fprintf(fid, ...
        "Reduced dual of KYP lemma (similarity)&%16.8g & %10.6f&%d&%d\\\\\n", ...
        Reduced_Dual_Esq,sol.solvertime,nnz(sol.solverinput.A), ...
        numel(sol.solverinput.A)); 
fclose(fid);
...
for N=10:10:80
  ...
  Objective=[-trace(Theta0*ZR)];
  if reduced_sol_problem == false
    reduced_sol=optimize(Constraints,Objective,Options);
  endif
  if reduced_sol.problem
    reduced_sol_problem = true;
    warning("YALMIP failed : %s",sol.info);
    break;
  else
    % Sanity checks
...
loglog(reduced_NN,reduced_numel_sedumi_A,"-",NN,numel_sedumi_A,"--");
legend("Reduced Dual KYP","Dual KYP");
legend("location","northwest");
legend("boxoff");
legend("left");
grid("on");
ylabel("numel(solverinput.A)");
xlabel("FIR filter order N");
title(sprintf("h=remez(N,2*[0,%4.2f,%4.2f,0.5],[1,1,0,0],[1,10]);",fap,fas));
print(strcat(strf,"_numel"),"-dpdflatex");
close;
