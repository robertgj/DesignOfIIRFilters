...
delete("allpass_MH2d_test.diary");
delete("allpass_MH2d_test.diary.tmp");
diary allpass_MH2d_test.diary.tmp
...
[b1,b2]=allpass_MH2d_pole2coef(r1,r2,"real");
[A,B,C,D,dAdx,dBdx,dCdx,dDdx]=allpass_MH2d_coef2Abcd(b1,b2);
if rank([A,B])~=rows(A)
...
allpass_filter_check_gradc1c2(@allpass_MH2d_coef2Abcd,w,b1,b2,del,del/200);
...
[b1,b2]=allpass_MH2d_pole2coef(r,theta,"complex");
[A,B,C,D,dAdx,dBdx,dCdx,dDdx]=allpass_MH2d_coef2Abcd(b1,b2);
if rank([A,B])~=rows(A)
  printf("rank([A,B])~=rows(A) (%d)\n",rank([A,B]));
endif
...
allpass_filter_check_gradc1c2(@allpass_MH2d_coef2Abcd,w,b1,b2,del,del/300);
...
[Hf,w]=freqz(b,a,1024);
[b1,b2]=allpass_MH2d_pole2coef(r,theta,"complex");
[A,B,C,D]=allpass_MH2d_coef2Abcd(b1,b2);
if rank([A,B])~=rows(A)
  printf("rank([A,B])~=rows(A) (%d)\n",rank([A,B]));
endif
H=Abcd2H(w,A,B,C,D);
...
[yMH2d,xxMH2d]=allpass_MH2d(b1,b2,u);
if max(abs(y-yMH2d)) > 500*tol
  error("max(abs(y-yMH2d)) > 500*tol");
endif

% Check noise gain
[yMH2df,xxMH2df]=allpass_MH2d(b1,b2,u,"round");
ngMH2d=allpass_MH2d_coef2ng(b1,b2)
est_varyd=(1+ngMH2d)/12
varyd=var(yMH2d-yMH2df)
...
% Check noise gain
[yMH2d,xxMH2d]=allpass_MH2d(b1,b2,u,"none");
[yMH2df,xxMH2df]=allpass_MH2d(b1,b2,u,"round");
ngMH2d=allpass_MH2d_coef2ng(b1,b2)
est_varyd=(1+ngMH2d)/12
varyd=var(yMH2d-yMH2df)
...
% Done
diary off
movefile allpass_MH2d_test.diary.tmp allpass_MH2d_test.diary;
