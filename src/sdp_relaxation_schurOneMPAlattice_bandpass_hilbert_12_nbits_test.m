...
unlink("sdp_relaxation_schurOneMPAlattice_bandpass_hilbert_12_nbits_test.diary");
unlink ...
  ("sdp_relaxation_schurOneMPAlattice_bandpass_hilbert_12_nbits_test.diary.tmp");
diary sdp_relaxation_schurOneMPAlattice_bandpass_hilbert_12_nbits_test.diary.tmp
...
Da0 = [   1.0000000000,  -0.7148803392,   1.0561010858,   0.2655316953, ... 
         -0.2174949906,   0.6788596440,  -0.0187354946,   0.1030989383, ... 
          0.1393383026,  -0.0002426340,   0.0310414150,   0.0181334617, ... 
         -0.0000856347 ]';
Db0 = [   1.0000000000,  -1.3822039202,   1.0543545655,   0.1452298499, ... 
         -0.5635123000,   0.6660814590,  -0.1478464943,   0.0288939866, ... 
          0.1464812323,  -0.0206950785,   0.0121567891,   0.0243313578, ... 
          0.0009546274 ]';
...
pdr=0.01 % Peak-to-peak phase ripple in multiples of pi radians
Wpp=100
...
xlabel("Frequency");
ylabel("Amplitude(dB)");
axis([min([fapl ftpl fppl]), max([fapu ftpu ftpu]), -0.1, 0.02]);
...
xlabel("Frequency");
ylabel("Phase(rad./pi)\n(corrected for delay)");
axis([min([fapl ftpl fppl]),max([fapu ftpu ftpu]),mod(pd-pdr,2),mod(pd+pdr,2)]);
...
% Save results
save sdp_relaxation_schurOneMPAlattice_bandpass_hilbert_12_nbits_test.mat ...
     ...
     Da0 Db0 A1k0 A1epsilon0 A1p0 A2k0 A2epsilon0 A2p0 ...
...
movefile ...
  sdp_relaxation_schurOneMPAlattice_bandpass_hilbert_12_nbits_test.diary.tmp ...
  sdp_relaxation_schurOneMPAlattice_bandpass_hilbert_12_nbits_test.diary;
