...
unlink("bitflip_bandpass_direct_test.diary");
unlink("bitflip_bandpass_direct_test.diary.tmp");
diary bitflip_bandpass_direct_test.diary.tmp

bitflip_bandpass_test_common;

function [cost,n,d,svec_out] = ...
         direct_cost(svec,_Ad,_Wa,_Td,_Wt,_n0,_d0,_nbits,_ndigits)
  ...
  elseif nargin ~= 1
    print_usage("[cost,n,d,svec_out] = ...\n\
      direct_cost(svec[,Ad,Wa,Td,Wt,n0,d0,nbits,ndigits])");
...
[cost_ex,n_ex,d_ex,svec_ex] = direct_cost([],Ad,Wa,Td,Wt,n0,d0,0,0)
...
% Rounded truncation
[cost_rd,n_rd,d_rd,svec_rd] = direct_cost([],Ad,Wa,Td,Wt,n0,d0,nbits,0)
h_rd=freqz(n_rd,d_rd,nplot);
t_rd=grpdelay(n_rd,d_rd,nplot);
% Find optimised coefficients with bit-flipping
svec_bf=bitflip(@direct_cost,svec_rd,nbits,bitstart,msize);
[cost_bf,n_bf,d_bf]=direct_cost(svec_bf)
h_bf=freqz(n_bf,d_bf,nplot);
t_bf=grpdelay(n_bf,d_bf,nplot);

% Signed-digit truncation
[cost_sd,n_sd,d_sd,svec_sd] = direct_cost([],Ad,Wa,Td,Wt,n0,d0,nbits,ndigits)
h_sd=freqz(n_sd,d_sd,nplot);
t_sd=grpdelay(n_sd,d_sd,nplot);
% Find optimised coefficients with bit-flipping and signed-digits
svec_bfsd=bitflip(@direct_cost,svec_sd,nbits,bitstart,msize);
[cost_bfsd,n_bfsd,d_bfsd]=direct_cost(svec_bfsd)
h_bfsd=freqz(n_bfsd,d_bfsd,nplot);
t_bfsd=grpdelay(n_bfsd,d_bfsd,nplot);

% Make a LaTeX table for cost
fname=sprintf("bitflip_bandpass_direct_test_cost.tab");
fid=fopen(fname,"wt");
...
fid=fopen("bitflip_bandpass_direct_test_adders.tab","wt");
...
axis([0 0.5 -60 10]);
grid("on");
tstr=sprintf("Bandpass direct form, nbits=%d,bitstart=%d,\
...
axis([0 0.5 0 25]);
grid("on");
print("bitflip_bandpass_direct_response","-dpdflatex");
close

% Print the results
print_polynomial(n0,"n0");
print_polynomial(n0,"n0",...
                 "bitflip_bandpass_direct_test_n0_coef.m");
print_polynomial(d0,"d0");
print_polynomial(d0,"d0",...
                 "bitflip_bandpass_direct_test_d0_coef.m");
print_polynomial(n_ex,"n_ex");
print_polynomial(n_ex,"n_ex",...
                 "bitflip_bandpass_direct_test_n_ex_coef.m");
print_polynomial(d_ex,"d_ex");
print_polynomial(d_ex,"d_ex",...
                 "bitflip_bandpass_direct_test_d_ex_coef.m");
print_polynomial(n_rd,"n_rd");
print_polynomial(n_rd,"n_rd",...
                 "bitflip_bandpass_direct_test_n_rd_coef.m",fmt_str);
print_polynomial(d_rd,"d_rd");
print_polynomial(d_rd,"d_rd",...
                 "bitflip_bandpass_direct_test_d_rd_coef.m",fmt_str);
print_polynomial(n_bf,"n_bf");
print_polynomial(n_bf,"n_bf",...
                 "bitflip_bandpass_direct_test_n_bf_coef.m",fmt_str);
print_polynomial(d_bf,"d_bf");
print_polynomial(d_bf,"d_bf",...
                 "bitflip_bandpass_direct_test_d_bf_coef.m",fmt_str);
print_polynomial(n_sd,"n_sd");
print_polynomial(n_sd,"n_sd",...
                 "bitflip_bandpass_direct_test_n_sd_coef.m",fmt_str);
print_polynomial(d_sd,"d_sd");
print_polynomial(d_sd,"d_sd",...
                 "bitflip_bandpass_direct_test_d_sd_coef.m",fmt_str);
print_polynomial(n_bfsd,"n_bfsd");
print_polynomial(n_bfsd,"n_bfsd",...
                 "bitflip_bandpass_direct_test_n_bfsd_coef.m",fmt_str);
print_polynomial(d_bfsd,"d_bfsd");
print_polynomial(d_bfsd,"d_bfsd",...
                 "bitflip_bandpass_direct_test_d_bfsd_coef.m",fmt_str);

% Save the results
save bitflip_bandpass_direct_test.mat n0 d0 n_ex d_ex n_rd d_rd ...
     n_bf d_bf n_sd d_sd n_bfsd d_bfsd

% Done
diary off
movefile bitflip_bandpass_direct_test.diary.tmp ...
         bitflip_bandpass_direct_test.diary;
