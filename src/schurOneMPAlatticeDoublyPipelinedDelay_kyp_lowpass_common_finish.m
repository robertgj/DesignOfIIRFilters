...
grid("on");
tstr=...
  sprintf("Amplitude error of parallel all-pass filter and delay : N=%d, DD=%d",
          N, DD);
title(tstr);
...
ax=plotyy(1:length(list_norm_dk),list_norm_dk,1:length(list_Esq),list_Esq);
ylabel(ax(1),"norm(dk)");
ylabel(ax(2),"Esq");
xlabel("Iteration");
axis(ax(1),[0 0.5 0 100]);
axis(ax(2),[0 0.5 0 100]);
grid("on");
tstr=sprintf("Convergence of parallel all-pass filter and delay : N=%d, DD=%d",
             N, DD);
title(tstr);
