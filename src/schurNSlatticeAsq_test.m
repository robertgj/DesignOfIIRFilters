...
if max(abs((abs(h).^2)-Asq)) > 32*eps
  error("max(abs((abs(h).^2)-Asq)) > 32*eps");
endif
...
if max(abs(diff_Asq-gradAsq(ntpl,:))) > del/928.96
  error("max(abs(diff_Asq-gradAsq(ntpl,:))) > del/928.96");
endif
...
if max(abs(diff_gradAsq-diagHessAsq(ntpl,:))) > del/214.82
  error("max(abs(diff_gradAsq-diagHessAsq(ntpl,:))) > del/214.82");
endif
