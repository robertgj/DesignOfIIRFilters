function genTex3_4(param,CompResult, InfoSDP, Errors, DefaultSeed, density)
solver = param.SDPsolver;
DENSITY = num2str(density);
fname = strcat('sec3.4_table_density', DENSITY, '.tex');
fid   = fopen(fname, 'w+');
genTable(fid, param, CompResult, InfoSDP, Errors, density);
fclose(fid);
N = size(CompResult, 3);
for j=1:N
	if j > 12
	...
	iname = strcat('sec3.4_info_table_sedumi_nDim', NDIM, '_density', DENSITY, '.tex');
	iid   = fopen(iname, 'w+');
	genInfoTables(iid, param, CompResult, InfoSDP, Errors, 'sedumi', DefaultSeed, j, density);
	fclose(iid);
	ename = strcat('sec3.4_error_table_sedumi_nDim', NDIM, '_density', DENSITY, '.tex');
	eid   = fopen(ename, 'w+');
	tmpaultSeed = genErrorTables(eid, param, CompResult, InfoSDP, Errors, 'sedumi', DefaultSeed, j, density);
	fclose(eid);
	iname = strcat('sec3.4_info_table_sdpt3_nDim', NDIM, '_density', DENSITY, '.tex');
	iid   = fopen(iname, 'w+');
	genInfoTables(iid, param, CompResult, InfoSDP, Errors, 'sdpt3', DefaultSeed, j, density);
	fclose(iid);
	ename = strcat('sec3.4_error_table_sdpt3_nDim', NDIM, '_density', DENSITY, '.tex');
	eid   = fopen(ename, 'w+');
	tmpSeed = genErrorTables(eid, param, CompResult, InfoSDP, Errors, 'sdpt3', DefaultSeed, j, density);
	fclose(eid);
end
return

function genTable(fid, param, CompResult, InfoSDP, Errors, density)
backS = char(92); % char(165);
aand = char(38);
fprintf(fid,'%sbegin{table}[htdp]\n',backS);
caption = 'Information%on%SDP%relaxation%problems%in%Subsection%3.4%with%density%';
caption = strcat(caption, num2str(density));
...
fprintf(fid, '%shline\n', backS);
fprintf(fid, '%smulticolumn{2}{|c|}{}%sSparse%sAdaptive SOS%s%s%s\n', backS, aand, aand, aand, backS, backS);
fprintf(fid, '%shline\n', backS);
...
label = strcat('subsec3.4_table_density', num2str(density));
fprintf(fid, '%slabel{%s}\n', backS,label);
fprintf(fid, '%send{table}\n', backS);
return

function seed = genInfoTables(fid, param, CompResult, InfoSDP, Errors, solver, DefaultSeed, idx, density)
bS = char(92);
aand = char(38);
fprintf(fid, '%sbegin{table}[htdp]\n', bS);
if idx > 12
	NDIM = num2str(10*(idx-6));
else
	NDIM = num2str(5*idx);
end
DENSITY = num2str(density);
caption = strcat('Numerical%results%on%SDP%relaxation%problems');
caption = strcat(caption, '%for%POPs%with%$n=', NDIM, '$%and%density%', DENSITY, '%in%Subsection%3.4%by%');
...
for j=1:NN
	...
	for SW=1:2
		...
		if SW == 1
			fprintf(fid, ' %5d %s Sparse %s ', seed, aand, aand);
		elseif SW == 2
...
fprintf(fid, '%send{tabular}\n', bS);
fprintf(fid, '}\n');
fprintf(fid, '%slabel{subsec3.4_info_table_%s_nDim%s_density%s}\n', bS, solver, NDIM, DENSITY);
fprintf(fid, '%send{center}\n', bS);
fprintf(fid, '%send{table}\n', bS);
return

function seed = genErrorTables(eid, param, CompResult, InfoSDP, Errors, solver, DefaultSeed, idx, density)
bS = char(92); % char(165);
aand = char(38);
...
if strcmp(solver, 'sedumi')
	...
	caption = strcat(caption, 'for%POPs%with%$n=', NDIM, '$%and%density%', DENSITY);
	caption = strcat(caption, '%in%Subsection%3.4%by%');
	caption = strcat(caption, 'SeDuMi');
	solvers = 1;
elseif strcmp(solver, 'sdpt3')
	...
	solvers = 2;
elseif strcmp(solver, 'sdpa')
	caption = 'Iter,%phasevaluee,%and%DIMACS%errors%for%SDP%relaxation%problems%';
	caption = strcat(caption, 'for%POPs%with%$n=', NDIM, '$%and%density%', DENSITY);
	caption = strcat(caption, '%in%Subsection%3.4%by%');
	caption = strcat(caption, 'SDPA');
	solvers = 3;
...
for j=1:NN
	...
	for SW=1:2
		...
		nerr = Errors{solvers, SW, idx, j}.nerr;
        	nerr = Errors{solvers, SW, idx, j}.nerr;
		if isnan(Errors{solvers, SW, idx, j}.dimacs(1))
			err1 = []; err2 = []; err3 = [];
			err4 = []; err5 = []; err6 = [];
		else
		...
		if SW == 1
 			if strcmp(solver, 'sdpa') == 0
	       		fprintf(eid,'%5d%s Sparse %s %2d %s %2d %s',seed, aand, aand,iter, aand, nerr, aand);
			else
	       		fprintf(eid,'%5d%s Sparse %s %2d %s %s %s',seed, aand, aand,iter, aand, nerr, aand);
			end
		elseif SW == 2
 			if strcmp(solver, 'sdpa') == 0
...
fprintf(eid, '%slabel{subsec3.4_error_table_%s_nDim%s_density%s}\n', bS, solver, NDIM, DENSITY);
