function errorTexEq(param, probNumbers, problemList, compResult, sedumiInfoVec, Esize, fcname)
...
%%%-->
fprintf(fcid, '%sbegin{tabular}{|c|cc|ccc|cccccc|}\n', backS);
fprintf(fcid, '%shline\n', backS);
fprintf(fcid, 'Problem%s {%ssmall (rSW, eSW)} %s $r$ %s Iter%s n.e.%s feasratio%s',aand,backS,aand, aand,aand,aand,aand);
...
for j=1:Esize
	...
	if strcmp(param.SDPsolver, 'sedumi')
		fprintf(fcid,'%s (%d, %d) %s %d %s',aand,reduceEqualitiesSW, elimFrSW,aand,ro,aand);
		fprintf(fcid,'%2d %s %d %s %2.1e %s',iter, aand, nerr, aand, feas, aand);
	elseif strcmp(param.SDPsolver, 'sdpa')
		fprintf(fcid,'%s (%d, %d) %s %2d %s%d%s',aand,reduceEqualitiesSW, elimFrSW,aand,ro,aand,iter, aand);
		status = statusOfSDPS(nerr);
		fprintf(fcid,'{%ssmall %s} %s %2.1e %s',backS,status, aand, feas, aand);
	end
