function errorTex(param, Solvers, probNumbers, problemList, compResult, sedumiInfoVec, Esize, fcname)
...
for kkk=1:length(probNumbers)
    ...
    if ~isempty(fileName)
        ...
        %%%-->
        outputResults(fcid, param, Solvers, compResult, sedumiInfoVec, fileName, Esize, i);
        %%%<--
    end
    if mod(i,Psize) == 0
...
%%%-->
caption = 'Iter,%numerr,%feasratio%and%dimacs%errors';
...
%%%-->
fprintf(fcid, '%sbegin{tabular}{|c|c|c|cc|cccccc|}\n', backS);
fprintf(fcid, '%shline\n', backS);
fprintf(fcid, 'Problem%s $r$ %ssolver%s Iter%s n.e.%s ',aand,aand,aand,aand,aand);
...
function outputResults(fcid, param, Solvers, compResult, sedumiInfoVec, fileName, Esize, i)
backS = char(92); % char(165);
aand = char(38);
for j=1:Esize
    if j == 1
    ...
    if strcmp(Solvers{j}, 'sedumi')
        fprintf(fcid,'%s %d %s {%ssmall %s} %s %2d %s',aand, ro,aand,backS,Solvers{j},aand,iter, aand);
        fprintf(fcid,'%d %s ',nerr, aand);
    elseif strcmp(Solvers{j}, 'sdpa')
        fprintf(fcid,'%s %d %s {%ssmall %s} %s %2d %s',aand, ro,aand,backS,Solvers{j},aand,iter, aand);
        status = statusOfSDPS(nerr);
        fprintf(fcid,'{%ssmall %s} %s ',backS,status, aand);
    end
...
%%%-->
label = strcat('DimacsErrors', num2str(ceil(i/Psize)));
%%%<--
