...
for j=1:lenBolckStruct
  fprintf(fileId,'%d ',full(SDPA.blockStruct(j)));
end
fprintf(fileId, '\n');
  
for j=1:lenBvect
  fprintf(fileId,'%+15.10f ',full(SDPA.bVect(1,j)));
end
fprintf(fileId,'\n'); 

for i=1:rowSize
  fprintf(fileId,'%6d ',full(SDPA.sparseMatrix(i,1)));
  fprintf(fileId,'%6d ',full(SDPA.sparseMatrix(i,2))); 
  fprintf(fileId,'%6d ',full(SDPA.sparseMatrix(i,3))); 
  fprintf(fileId,'%6d ',full(SDPA.sparseMatrix(i,4)));
  fprintf(fileId,'%+15.10f\n',full(SDPA.sparseMatrix(i,5)));
end
