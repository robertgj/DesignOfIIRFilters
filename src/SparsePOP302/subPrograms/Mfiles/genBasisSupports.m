function [basisSupports] = genBasisSupports(objPoly,ineqPolySys,...
    basisIndices,basisCliqueNo,param);
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%$%$ Kojima 08/05/2007 ---> 
% The choice of developmentSW = 3 is not so effective usually because
% it creates too many equalities. So this part needs to be
% investigated further. 
genBasisSW = 0; 
if param.reduceEqualitiesSW == 1 || param.reduceEqualitiesSW == 2
	fprintf('## genBasisSW = %d at genBasisSupports\n',genBasisSW); 
	if genBasisSW == 1 
		fprintf('## Generate more valid equalities than the original SDP relax.\n');
	end
end
%$%$ <--- Kojima 2007/08/05
nDim = objPoly.dimVar;
...
for i=1:m
	...
	if param.sparseSW == 2 || param.sparseSW == 3
		%
		% 2011-08-03 H.Waki
		% For new sdp relaxation
		%
		%maxDeg = ineqPolySys{i}.degree;
		%for j=2:size(ineqPolySys,2)
		%	if maxDeg < ineqPolySys{j}.degree
		%		maxDeg = ineqPolySys{j}.degree;
		%	end
		%end
		degree = ineqPolySys{i}.degree;
		tmpDeg = rO/degree -1/2;
		if (ineqPolySys{i}.typeCone == -1)
			if genBasisSW == 1
				sosDim = floor(2*tmpDeg); 
			else
				sosDim = 2*floor(tmpDeg); 
			end
		else
			sosDim = floor(tmpDeg);
		end	
		basisSupports{i} = genMultipleSupport(ineqPolySys{i}.supports, sosDim);
		%full(basisSupports{i})
		%sosDim
		%ineqPolySys{i}.typeCone
	    ...
	    if (ineqPolySys{i}.typeCone == -1)
        	...
        	%$%$ Kojima 08/05/2007 ---> 
	        if genBasisSW == 1 
    	        sosDimEquality = 2*rO - ineqPolySys{i}.degree; 
        	    basisSupports{i} = genSimplexSupport(1,nDim,sosDimEquality,...
...
function  Support = genMultipleSupport(ineqSupport, sosDim)

Support = ineqSupport;
nDim = size(ineqSupport,2);
if sosDim <= 0
	Support = sparse(1, nDim);
	return;
elseif sosDim == 1
	Support = [ineqSupport;sparse(1,nDim)];
	Support = my_unique(Support, 'rows');
	return;
end
tmpSupport = [ineqSupport;sparse(1,nDim)];
tmpSupport = my_unique(tmpSupport, 'rows');
tmpSupport0 = tmpSupport;
for i=1:sosDim-1
	tmpSupport = addSup(tmpSupport,tmpSupport0, 1);
end
Support = tmpSupport;
return
