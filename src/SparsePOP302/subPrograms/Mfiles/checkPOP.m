...
if isfield(param,'reduceMomentMatSW')
    if (param.reduceMomentMatSW ~= 0) && (param.reduceMomentMatSW ~= 1) && (param.reduceMomentMatSW ~= 2)
        fprintf('## param.reduceMomentMatSW=%d needs to be 0, 1 or 2##\n',param.reduceMomentMatSW);
        continueSW = 0;
    end
end
%{
...
if isfield(param,'SDPsolverSW')
    if (param.SDPsolverSW ~= 1) && (param.SDPsolverSW ~= 0);
        fprintf('## param.SDPsolverSW=%d needs to be 0 or 1 ##\n',param.SDPsolverSW);
        continueSW = 0;
    end
end
if isfield(param,'sparseSW')
	...
	%
    if (param.sparseSW ~= 0) && (param.sparseSW ~= 1) && (param.sparseSW ~= 2) && (param.sparseSW ~= 3)
        fprintf('## param.sparseSW=%d needs to be 0, 1, 2 or 3 ##\n',param.sparseSW);
        continueSW = 0;
    end
end
...
%	%disp(length(N));	
%else
	[temp, M, N] = my_unique(poly.supports,'rows');
%end
