...
void mono::setSuppDense(int i, int val){
	int length = Supp.size();
	if(length <= i){
		errorMsg("Error: setSuppDense");
		exit(EXIT_FAILURE);
	}else{
		//cout << "Supp["<< i << "] = " << Supp[i] << endl;
		Supp[i] = Supp[i] + val;
		//cout << "Supp["<< i << "] = " << Supp[i] << endl;
	}
...
void mono::convSuppDenseToSparse(){
...
int mono::getSupp(int noterm){
    if(noterm <0 || noterm >= nDim){
        errorMsg("## Access Error for mono::supVal");
        exit(EXIT_FAILURE);
    }
...
double mono::getCoef(int nof){
	...
	errorMsg("## index is out of range of Coef");
	exit(EXIT_FAILURE);
    }else{
        return Coef[nof];
    }
...
void mono::setSupp(int nov, int value){
    ...
    if(nov < 0 || nov >= nDim){
	errorMsg("## index is out of range of supports.");
        exit(EXIT_FAILURE);
    }else{
        ...
        if(value == 0){
			...
			*/
            ...
            errorMsg("## value must be nonnegative.");
            exit(EXIT_FAILURE);
        }
        //this->sortMono();
    }
...
void poly::addMono(class mono & monoNew){
	...
	errorMsg("## Cannot add mono @ addMono.");
	exit(EXIT_FAILURE);
}
int poly::Degree(){
    setDegree();
...
void poly::evalPoly(vector<double> const Var, vector<double> & fValue, vector<double> & maxAbs){
    ...
    if(Var.size()!=dimVar){
        errorMsg("## error:not num o variable == dimVar @evalPoly");
        exit(EXIT_FAILURE);
    }
    ...
    ///case of typeCone = 3
        ...
        errorMsg("## error@evalPolynomial: the typeCone is not available");
        exit(EXIT_FAILURE);
    }
...
class poly assignPolyToVar(class poly & Poly, vector<class poly> & polyVec, int nov){
    ...
    if(0 <= nov && nov<Poly.dimVar){
        ...
        errorMsg("error@assignPolytoVar: the NumverOfVariable is not included in the Polynomial");
        exit(EXIT_FAILURE);
    }
}
...
double bounds::lbd(int i){
    if(lower.empty()){
        ...
        errorMsg("## index is out of range of lower bounds.");
        exit(EXIT_FAILURE);
    }else{
        return lower[i];
    }
...
double bounds::ubd(int i){
    if(upper.empty()){
        ...
        errorMsg("## index is out of range of upper bounds.");
        exit(EXIT_FAILURE);
    }else{
        return upper[i];
    }
...
void polysystem::scalingPOP(vector<double> & pMatrix, vector<double> & bVect, vector<double> & scaleVect, int symbolicMath){
    ...
    for(int i=0;i<this->dimVar;i++){
        if(symbolicMath == 0){
            pMatrix[i] = 1.0;
            bVect[i] = 0.0;    
        }else if(MIN<bounds.lbd(i)&&bounds.ubd(i)<MAX){
            //pMatrix[i+i*this->dimVar] = bounds.ubd(i)-bounds.lbd(i);
            if(fabs(bounds.lbd(i) - bounds.ubd(i)) <EPS){
                pMatrix[i] = 1.0;
                //bVect[i] = 0.0;
	...
	//cout << " pMatrix[" << i << "] = " << pMatrix[i] << endl;
	//cout << "   bVect[" << i << "] = " << bVect[i] << endl;
    }
    
    //cout << "***Start of making VarInConst and VarInConstList" << endl;
    ...
    for(int i=0;i<dimVar;i++){
        ...
        if(symbolicMath == 0){
                ...
                boundsNew.setLow(i+1, bounds.lbd(i));
                if(bounds.ubd(i)<MAX && MIN<bounds.lbd(i)){
                    NumOfActiveBounds += 2;
                }else if(bounds.ubd(i)>=MAX && MIN<bounds.lbd(i)){
                    NumOfActiveBounds += 1;
                }else if(bounds.ubd(i)<MAX && MIN>=bounds.lbd(i)){
                    NumOfActiveBounds += 1;
                }
                ...
                if(fabs(bounds.lbd(i)) >= EPS || fabs(bounds.ubd(i)-1) >= EPS){
            		...
            		if(fabs(bounds.ubd(i) - bounds.lbd(i)) > EPS){
	...
	*/	
	...
	for(int j=0; j<dimVar ; j++){
		...
		if(MIN < boundsNew.lbd(j) && boundsNew.ubd(j) < MAX){
			if(fabs(boundsNew.lbd(j)) <EPS && fabs(boundsNew.ubd(j)) <EPS){
				absBound = 1.0;
			}else if(fabs(boundsNew.lbd(j)) < fabs(boundsNew.ubd(j))){
				absBound = fabs(boundsNew.ubd(j));
			}else{
				absBound = fabs(boundsNew.lbd(j));
			}
			//cout << "(j, absBound) = (" << j << ", " << absBound << ")" << endl;
            if(absBound > EPS){
                if(1 < absBound){
                    vscale[j] = absBound;
            ...
            pMatrix[j] *= vscale[j];
		//cout << " pMatrix[" << j << "] = " << pMatrix[j] << endl;
		//cout << "   bVect[" << j << "] = " << bVect[j] << endl;
        }
    }
    //system("top -b -n 1 | grep MATLAB | head -1 |awk '{printf(\"memory = %s\\n\"), $6}' ");
...
void poly::sumSupports(vector<int> & varList){
    varList.clear();
    list<mono>::iterator ite;
	...
	vector<int>::iterator newend = unique(varList.begin(), varList.end());
	varList.erase(newend, varList.end());
	/*
	cout << "sumSupports" << endl;
	for(vt=varList.begin();vt!=varList.end();++vt){
		cout << (*vt) << " ";
	}
	cout << endl;
	*/
...
void polysystem::boundToIneqPolySys(int typeExchange){
	...
	for(int i=0; i<posOflbds.size();i++){
		cout << "posOflbds[" << i << "]= "; 
		list<int>::iterator itb = posOflbds[i].begin();
		list<int>::iterator ite = posOflbds[i].end();
...
class poly polysystem::createUpLowPoly(int varNo, double lbd, double ubd){
    ...
    if(lbd<ubd){
        ...
        errorMsg("## error@createUPLOWPoly: not available ubd,lbd: lbd>ubd");
        exit(EXIT_FAILURE);
    }
}
...
int polysystem::polyIsBinary(int nop){
	if(nop<this->numsys()){
		int idx;
		idx = polynomial[nop].isBinary();
		return idx;
	}else{
		errorMsg("## error@polysystem::polyIsBinary :: Nop'th polynomial does not exist");
		exit(EXIT_FAILURE);
	}
	return -1;
}
int poly::isBinary(){
	if(typeCone==EQU && sizeCone == 1){
		if(monoList.size() ==2){
			...
			mono1 = (*ite);
			mono1.getSup(sup1);			
			nzSize1 = mono1.lengthNzSupp();
			coef1 = mono1.getCoef(0);
			++ite;

			mono2 = (*ite);
			mono2.getSup(sup2);
			nzSize2 = mono2.lengthNzSupp();
			coef2 = mono2.getCoef(0);
			/*
			cout << "coef1 = " << coef1 << endl;
			cout << "coef2 = " << coef2 << endl;
			cout << "sup1.val[0] = " << sup1.val[0] << endl;
			cout << "sup2.val[0] = " << sup2.val[0] << endl;
			cout << "sup1.idx[0] = " << sup1.idx[0] << endl;
			cout << "sup2.idx[0] = " << sup2.idx[0] << endl;
			*/
			if(nzSize1 == 1 && nzSize2 == 1){
				//mono1.writeMono();
				//mono2.writeMono();
				//cout << "Idx check" << endl;
				if(sup1.idx[0] == sup2.idx[0]){
					//cout << "Val check" << endl;
					if(sup1.val[0] == 1 && sup2.val[0] == 2){
						//cout << "Coef check" << endl;
						if(fabs(coef1+1) < EPS && fabs(coef2-1) < EPS){
							//cout << "binIdx = " << sup1.idx[0] << endl;
							return sup1.idx[0];
						}else if(fabs(coef2+1) < EPS && fabs(coef1-1) < EPS){
							//cout << "binIdx = " << sup1.idx[0] << endl;
							return sup1.idx[0];
						}
					}else if(sup1.val[0] == 2 && sup2.val[0] == 1){
						//cout << "Coef check" << endl;
						if(fabs(coef1+1) < EPS && fabs(coef2-1) < EPS){
							//cout << "binIdx = " << sup1.idx[0] << endl;
							return sup1.idx[0];
	...
	return -1;//This constraint is not the form of xi^2 -xi = 0;
}
int polysystem::polyIsSquareOne(int nop){
	if(nop<this->numsys()){
		int idx;
		idx = polynomial[nop].isSquareOne();
		return idx;
	}else{
		errorMsg("## error@polysystem::polyIsSquareOne :: Nop'th polynomial does not exist");
		exit(EXIT_FAILURE);
	}
	return -1;
}
int poly::isSquareOne(){
	if(typeCone==EQU && sizeCone == 1){
		if(monoList.size() ==2){
			class mono mono1, mono2;
			class sup sup1, sup2;	
	...
	return -1;//This constraint is not the form of xi^2 -1 = 0;
}
int polysystem::polyIsComplimentarity(int nop, class sup & czSup, double & Coef) {
    if(nop<this->numsys()){
        ...
        errorMsg("## error@polysystem::polyIsComplimentarity :: Nop'th polynomial does not exist");
        exit(EXIT_FAILURE);
    }
    return NO;
}
...
class mono multiMonos(class mono & Mono1, class mono & Mono2){
    ...
    if(Mono1.lengthCoef()==Mono2.lengthCoef()){
        ...
        errorMsg("## error@multiMonos: Length of Coef vectors is not same");
        exit(EXIT_FAILURE);
    }
    ...
    if(Mono1.lengthSupp()==Mono2.lengthSupp()){
        ...
        errorMsg("error@multiMonos: Length of Support vectors is not same");
        exit(EXIT_FAILURE);
    }
...
class poly multiPolys(class poly & Poly1, class poly & Poly2){
    ...
    if(Poly1.dimvar()!=Poly2.dimvar()){
        errorMsg("## error@sumPoly: not same Dimension of Variables of Poly1 and Poly2");
        exit(EXIT_FAILURE);
    }
    if(Poly1.lengthCoef()!=Poly2.lengthCoef()){
        errorMsg("error@sumPoly: not same Length of Coeffient Vector of Poly1 and Poly2");
        exit(EXIT_FAILURE);
    }
...
class poly powPoly(class poly & Poly, int k){
    ...
    if(k==0){
        ...
        errorMsg("error@powPoly:this functon cannot permit you to caluculate Pow(*,k) <- k>=1");
        exit(EXIT_FAILURE);
    }
}
...
#ifndef MATLAB_MEX_FILE
void polysystem::setMinOrMax(int val){
	if(val != 1 && val != -1){
		errorMsg("# Error: minOrmax should be 1 or -1.");
		exit(EXIT_FAILURE);
	}else{
		minOrmax = val;
	}
}
int polysystem::getMinOrMax(){
	return minOrmax;
}
#endif /* ifndef MATLAB_MEX_FILE */
/* 
void polysystem::setSizeRemIdx(int no){
	if(no > 0){
		removeIdx.resize(no,-1);
	}else{
		cout << "# Error: size of removeIdx should be positve integer."<< endl;
	}
}
*/
void polysystem::removeEQU(vector<int> & remainIdx){
	if(removeIdx.empty() == false){
		int size = numsys();
		int rsize= removeIdx.size();
		int redsize= size - rsize;
		numSys = redsize;		
		remainIdx.resize(redsize,-1);
		vector<int> origIdx;
		origIdx.resize(size,-1);
		for(int i=0; i<size; i++){
			origIdx[i] = i;
		}
		//sort(removeIdx.begin(), removeIdx.end());
		bool flag = true;
		int k = 0;
		for(int i=0; i<size; i++){
			for(int j=0; j<rsize; j++){
				if(origIdx[i] == removeIdx[j]){
					flag = false;
					break;
				}
			}
		...
		vector<class poly> redPolyVec;
		redPolyVec.resize(redsize);
		vector<int>::iterator it;
		k=0;
		for(it = remainIdx.begin(); it != remainIdx.end(); ++it){
			redPolyVec[k] = polynomial[(*it)];
			k++;
		}
		...
		polynomial.clear();
		polynomial.resize(redsize);
		for(int j=0; j<redsize; j++){
			polynomial[j] = redPolyVec[j];
			polynomial[j].noSys = j;
			//polynomial[j].writePolyData();
		}	
		int tmpval = 0;
		list<int> lbd;
		for(int i=0; i<posOflbds.size();i++){
			...
			for(list<int>::iterator it=itb;it!=ite; ++it){
				tmpval = (*it) - rsize;
				lbd.push_back(tmpval);
				//posOflbds[i].pop_front();
				//posOflbds[i].push_back(tmpval);	
				//cout << (*it) << " ";
			}
			posOflbds[i].clear();
			posOflbds[i] = lbd;
		...
		list<int> ubd;
		for(int i=0; i<posOfubds.size();i++){
			list<int>::iterator itb = posOfubds[i].begin();
			list<int>::iterator ite = posOfubds[i].end();
			for(list<int>::iterator it=itb;it!=ite; ++it){
				tmpval = (*it) - rsize;
				ubd.push_back(tmpval);
				//posOfubds[i].pop_front();
				//posOfubds[i].push_back(tmpval);	
				//cout << (*it) << " ";
			}
			posOfubds[i].clear();
			posOfubds[i] = ubd;
...
int polysystem::checkBMI(){
	// 1 if POP contains a Polynomial SDP constraint or a SOCP constraint. 
	// 0 otherwise.
	int size = numsys();
	int typeCone;
	for(int i=0; i<size; i++){
		typeCone = polynomial[i].typecone();
		if(typeCone == 2 || typeCone == 3){
			return 1;
		}
	}
	return 0;
