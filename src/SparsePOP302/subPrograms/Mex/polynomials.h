...
#ifndef MATLAB_MEX_FILE
	void setSuppDense(int i, int val);
	void convSuppDenseToSparse();
#endif /* ifnded MATLAB_MEX_FILE */
...
public:
	...
	*/
	...
	void sumSupports(vector<int> & sumSupp);
...
public:
	...
	#ifndef MATLAB_MEX_FILE
	int minOrmax;	// minOrmax = 1 if minimization
			// minOrmax = -1 if maximization	
	void setMinOrMax(int val);
	int  getMinOrMax();
	#endif /* ifndef MATLAB_MEX_FILE */ 
	vector<int> removeIdx;
	//void setSizeRemIdx(int no);
    void removeEQU(vector<int> & remainIdx);
	//constructor
	polysystem(){
		numSys = -1;
		dimVar = -1;
		objConst = 0;
		#ifndef MATLAB_MEX_FILE
		minOrmax = 1;//default is minimization
		#endif /* ifndef MATLAB_MEX_FILE */ 
	};
	/*
	...
	*/
	...
	int polyIsBinary(int nop);
	int polyIsSquareOne(int nop);
	...
	void scalingPOP(vector<double> & pMatrix, vector<double> & bVect, vector<double> & scaleVect, int symbolicMath);
