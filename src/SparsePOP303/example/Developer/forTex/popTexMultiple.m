function popTexMultiple(param, Solvers, probNumbers, problemList, compResult, Esize, fpname)
...
for kkk=1:length(probNumbers)
    ...
    if ~isempty(fileName)
        ...
        %%%-->
        outputResults(fpid, Solvers, compResult, fileName, Esize, i);
        %%%<--
    end
    if mod(i,Psize) == 0
...
%%%-->
fprintf(fpid, '%sbegin{tabular}{|c|c|c|cccc|ccc|}\n', backS);
fprintf(fpid, '%shline\n', backS);
fprintf(fpid, 'Problem%s $r$%ssolver%sSDPobj%s POPobj%s absE%s ',aand,aand,aand,aand,aand,aand);
...
function outputResults(fpid, Solvers, compResult, fileName, Esize, i)
backS = char(92); % char(165);
aand = char(38);
for j=1:Esize
    if j == 1
    ...
    cpuL= full(compResult{j}(i,6));
    objPL= full(compResult{j}(i,13));
    errL= full(compResult{j}(i,8));
    objS= full(compResult{j}(i,9));
    objP= full(compResult{j}(i,10));
    err = full(compResult{j}(i,12));
    fprintf(fpid, '%s  %d %s {%ssmall %s} %s', aand,  ro, aand, backS, Solvers{j}, aand);
...
%%%-->
label = strcat('POPsolver', num2str(ceil(i/Psize)));
%%%<--
