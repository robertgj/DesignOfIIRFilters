...
if isempty(filename) || nargin < 3
	param0.printFileName = strcat('tmpsp303.dev_mex',num2str(param0.mex),'_',param0.errorBdIdx,'_', param0.POPsolver,'.out');
else
...
elseif strcmp(param.SDPsolver, 'sdpt3')
	...
	errs = SeDuMiInfo.dimacs';
elseif strcmp(param.SDPsolver, 'sdpnal') || strcmp(param.SDPsolver, 'sdpnalplus')
	iter = SeDuMiInfo.iter;
	nerr = SeDuMiInfo.numerr;
	%feas = max(SeDuMiInfo.errs(1), SeDuMiInfo.errs(3));
	%errs = SeDuMiInfo.errs;
	feas = 0; errs = zeros(1, 6);
end
