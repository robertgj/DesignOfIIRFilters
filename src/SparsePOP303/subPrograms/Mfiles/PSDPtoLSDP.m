...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [SDPA,xIdxVec] = PSDPtoLSDP(objPoly,inEqPolySys,basisSupports,boundList,CompSup,ConstraintInfo, param)
...
squareOneIdx = ConstraintInfo.Term2.combsquare;
binaryIdx = ConstraintInfo.Term2.combbinary;
inEqIdx = setdiff((1:noOfInequalities),CompConst);
inEqIdx = setdiff(inEqIdx, squareOneIdx);
inEqIdx = setdiff(inEqIdx, binaryIdx);
...
%
SquareOneVec = [];
if param.SquareOneSW == 1 && ~isempty(squareOneIdx)
	for i=squareOneIdx
		[rowIdx, colIdx] = find(inEqPolySys{i}.supports);
		SquareOneVec = [SquareOneVec, colIdx]; 
	end
end
%
% 2011-04-29 H. Waki
...
%
BinaryVec = [];
if param.binarySW == 1 && ~isempty(binaryIdx)
	for i=binaryIdx
		[rowIdx, colIdx] = find(inEqPolySys{i}.supports);
		BinaryVec = [BinaryVec, colIdx(1)]; 
	end
end
...
if noOfBasisSupports > noOfInequalities
    for k=(noOfInequalities+1):noOfBasisSupports
		...
		% 2009-12-21 H. Waki
		%
		if param.SquareOneSW == 1 && ~isempty(SquareOneVec) && ~isempty(temp)
			temp = substituteSqOne(temp, SquareOneVec);
		end
		%
		% 2011-04-29 H. Waki
		%
		if param.binarySW == 1 && ~isempty(BinaryVec) && ~isempty(temp)
			temp = substituteBinary(temp, BinaryVec);
		end
		%full(temp)
		%blockSt
        if ~isempty(temp)
...
[Out, M,N] = my_unique(monoList,'rows');
...
sparseMatIndices = find(varList(:,nDim+1) > 0);
matrixNumber = varNumber(sparseMatIndices)';
blockNumber = varList(sparseMatIndices,nDim+1);
rowIndex = varList(sparseMatIndices,nDim+2);
colIndex = varList(sparseMatIndices,nDim+3);
value = varList(sparseMatIndices,nDim+4);
...
%{
	SDPA.sparseMatrix(:,2:5) = varList(sparseMatIndices, nDim+(1:4));
	SDPA.sparseMatrix(:,1) = matrixNumber;
	value = SDPA.sparseMatrix(I, 5);
	SDPA.sparseMatrix(I, 5) = -value;
%}
    J = find(matrixNumber);
