...
Hf=freqz(f1,1,wa);
subplot(211);
plot(wa(1:nFap)*0.5/pi,abs(Hf(1:nFap)));
axis([0 Fap 1-delta_p 1]);
ylabel("Magnitude");
grid("on");
title("Johansson-and-Saram\\\"{a}ki FIR pass-band and stop-band responses");
subplot(212);
plot(wa(nFas:end)*0.5/pi,abs(Hf(nFas:end)));
axis([Fas 0.5 0 delta_s]);
ylabel("Magnitude");
xlabel("Frequency");
grid("on");
...
close
ax=plotyy(wa*0.5/pi,Fr,wa(nasl:nasu)*0.5/pi,Fr(nasl:nasu));
set(ax(1),'ycolor','black');
set(ax(2),'ycolor','black');
