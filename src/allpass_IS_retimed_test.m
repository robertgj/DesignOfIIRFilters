...
unlink("allpass_IS_retimed_test.diary");
unlink("allpass_IS_retimed_test.diary.tmp");
diary allpass_IS_retimed_test.diary.tmp
...
allpass_filter_check_gradc1c2(@allpass_IS_retimed_coef2Abcd,w,d1,d2,del,del/80);
...
allpass_filter_check_gradc1c2(@allpass_IS_retimed_coef2Abcd,w,d1,d2,del,del/100);
...
[Hf,w]=freqz(b,a,1024);
[d1,d2]=allpass_IS_retimed_pole2coef(r,theta,"complex");
[A,B,C,D]=allpass_IS_retimed_coef2Abcd(d1,d2);
if rank([A,B])~=rows(A)
  printf("rank([A,B])~=rows(A) (%d)\n",rank([A,B]));
endif
H=Abcd2H(w,A,B,C,D);
...
% Check noise gain
[yIS_retimed,xxIS_retimed]=allpass_IS_retimed(d1,d2,u);
if max(abs(y-yIS_retimed)) > 500*tol
  error("max(abs(y-yIS_retimed)) > 500*tol");
endif
[yIS_retimedf,xxIS_retimedf]=allpass_IS_retimed(d1,d2,u,"round");
ngIS_retimed=allpass_IS_retimed_coef2ng(d1,d2)
est_varyd=(1+ngIS_retimed)/12
varyd=var(yIS_retimed-yIS_retimedf)
...
% Done
diary off
movefile allpass_IS_retimed_test.diary.tmp allpass_IS_retimed_test.diary;
