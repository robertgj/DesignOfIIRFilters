...
D1_0 = [  1.0000000000,  -1.3565689205,   1.0216861138,   0.9231820665, ... 
         -2.0515691527,   1.9451791072,  -0.3761463981,  -0.6600606495, ... 
          0.8765907243,  -0.4323253187,   0.1237452172 ]';
D2_0 = [  1.0000000000,  -2.0191595226,   1.4259656898,   1.1340052465, ... 
         -2.9463765639,   2.4736331548,  -0.3832259845,  -0.9822125577, ... 
          1.0882281850,  -0.5116335824,   0.1272747047 ]';
...
if use_best_branch_and_bound_found
  if branch_bound_schurOneMPAlattice_bandpass_hilbert_12_nbits_test_allocsd_Ito
    branches_min=194; % Esq_min=0.00150648
    A1k_min = [     -792,     1152,     1104,    -1024, ... 
                    1584,     -635,     -128,      896, ... 
                    -542,      240 ]'/2048;
    A2k_min = [    -1576,     1376,     1088,    -1144, ... 
                    1602,     -560,     -120,      896, ... 
                    -513,      264 ]'/2048;
  elseif ...
    branch_bound_schurOneMPAlattice_bandpass_hilbert_12_nbits_test_allocsd_Lim
    branches_min=168; % Esq_min=0.0015663
    A1k_min = [     -784,     1104,     1152,    -1024, ... 
                    1568,     -592,     -112,      904, ... 
                    -532,      253 ]'/2048;
    A2k_min = [    -1592,     1392,     1072,    -1144, ... 
                    1600,     -536,      -96,      908, ... 
                    -496,      272 ]'/2048;
  else
    branches_min=156 %Esq_min=0.00483012
    A1k_min = [     -800,     1312,      864,     -968, ... 
                    1664,     -776,      208,      994, ... 
                    -632,      440 ]'/2048;
    A2k_min = [    -1600,     1472,      864,    -1052, ... 
                    1664,     -704,      252,      976, ... 
                    -608,      472 ]'/2048;
  endif
...
if ~improved_solution_found
  ...
  P=schurOneMPAlatticeP ...
      (wp,A1k_min,A1epsilon0,A1p_ones,A2k_min,A2epsilon0,A2p_ones,difference);
  vPl=local_max(Pdl-P);
  vPu=local_max(P-Pdu);
  wPS=sort(unique([wp(vPl);wp(vPu);wp([1,end])]));
  PS=schurOneMPAlatticeP ...
       (wPS,A1k_min,A1epsilon0,A1p_ones,A2k_min,A2epsilon0,A2p_ones,difference);
  ...
  T_kmin=schurOneMPAlatticeT ...
           (wt,A1k_min,A1epsilon0,A1p_ones,A2k_min,A2epsilon0,A2p_ones,...
               difference);
  P_k0=schurOneMPAlatticeP(wp,A1k0,A1epsilon0,A1p0,A2k0,A2epsilon0,A2p0,...
               difference);
  P_k0_sd=schurOneMPAlatticeP ...
           (wp,A1k0_sd,A1epsilon0,A1p_ones,A2k0_sd,A2epsilon0,A2p_ones,...
               difference);
  P_kmin=schurOneMPAlatticeP ...
           (wp,A1k_min,A1epsilon0,A1p_ones,A2k_min,A2epsilon0,A2p_ones,...
               difference);
